...
const stub = {
  name: 'stub',
  setup(b) {
    b.onResolve({ filter: /(\?raw$)|(\/setup$)|(\/storage$)|(api\/index$)/ }, (a) => ({
      path: a.path,
      namespace: 'stub',
    }));
...
export default guard; export const loadJSON=()=>null; export const saveJSON=()=>true; export const loadVersioned=()=>null; export const saveVersioned=()=>true; export const SCHEMA_VERSION_FIELD='__schemaVersion';`,
      loader: 'js',
    }));
  },
};
...
const res = await build({
  entryPoints: [join(__dir, '..', 'src', 'qa', 'findings.ts')],
  bundle: true,
  format: 'esm',
  write: false,
  platform: 'node',
...
const file = join(tmp, 'findings.mjs');
writeFileSync(file, res.outputFiles[0].text);
const m = await import('file://' + file.replace(/\\/g, '/'));
if (typeof m.fingerprint !== 'function')
  throw new Error('fingerprint not exported from findings.ts bundle');
...
const res2 = await build({
  entryPoints: [join(__dir, '..', 'src', 'qa', 'securityReport.ts')],
  bundle: true,
  format: 'esm',
  write: false,
  platform: 'node',
...
const file2 = join(tmp, 'securityReport.mjs');
writeFileSync(file2, res2.outputFiles[0].text);
const mr = await import('file://' + file2.replace(/\\/g, '/'));
if (typeof mr.reportToSarif !== 'function')
  throw new Error('reportToSarif not exported from securityReport.ts bundle');
