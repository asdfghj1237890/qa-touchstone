...
async function main() {
  ...
  const scene = async (name, fn) => {
    try { if (fn) await fn(); await sleep(450); await shot(name); }
    catch (e) { console.error('  ! scene failed:', name, '—', e.message); }
  };
  ...
  await scene('02-security-matrix', async () => {
    ...
    if (!info || info.endpoints < 4) throw new Error('httpbin endpoints not found in demo collection (got ' + (info && info.endpoints) + ')');
    await nav('Security');
  ...
  await scene('05-api-docs', async () => { await nav('API Docs'); await sleep(600); });

  // 6. Performance / load testing
  await scene('06-performance', async () => { await nav('Performance'); await sleep(600); });

  // 7. Realtime (WS / SSE)
  await scene('07-realtime', async () => { await nav('Realtime'); await sleep(600); });

  // 8. Monitors (scheduled collection runs)
  await scene('08-monitors', async () => { await nav('Monitors'); await sleep(600); });
