...
function FilesPage() {
  ...
  const [pushDestPath, setPushDestPath] = useState('/vendor/etc/halo_config/core-plugins/sidewalk.conf.d');
  ...
  const handleDeviceTypeChange = (event, newDeviceType) => {
    if (newDeviceType !== null) {
      ...
      if (newDeviceType === 'efd-fos') {
        ...
        setPushDestPath('/vendor/etc/halo_config/core-plugins/sidewalk.conf.d');
        ...
        setPushDestPath('/etc/halo_config/core-plugins/sidewalk.conf.d');
        // If currently on serial tabs, switch to pull files tab
        if (activeTab === 2 || activeTab === 3) {
          setActiveTab(0);
        }
