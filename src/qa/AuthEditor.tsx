...
function OAuth2Editor({ a, set, token, onFetch }: OAuth2EditorProps) {
  ...
  const setO = (field: keyof QaAuth['oauth2'], val: string) => set('oauth2', { ...o, [field]: val });
  ...
  const run = async () => {
    ...
    try { await onFetch(); }
    catch (e: any) { setError(String(e && e.message ? e.message : e)); }
    finally { setBusy(false); }
  };
  return (
    <div className="qa-oauth">
      <FieldRow label={t('request.oauth.grantType')}>
        <Dropdown value={grant} options={OAUTH_GRANTS.map(g => ({ value: g.value, label: t(g.labelKey) }))} onChange={(v: string) => setO('grant', v)} />
      </FieldRow>
      {grant === 'authorization_code' && (
        <>
          ...
          <FieldRow label={t('request.oauth.authorizationCode')}><SecretInput value={o.code || ''} placeholder="code from callback"
            onChange={(v: string) => setO('code', v)} /></FieldRow>
      ...
      <div className="qa-field-grid">
        ...
        <FieldRow label={t('request.oauth.clientSecret')}><SecretInput value={o.clientSecret || ''} placeholder="••••••••"
          onChange={(v: string) => setO('clientSecret', v)} /></FieldRow>
      </div>
      ...
      {grant === 'password' && (
        <div className="qa-field-grid">
          ...
          <FieldRow label={t('request.auth.password')}><SecretInput value={o.password || ''} placeholder="password"
            onChange={(v: string) => setO('password', v)} /></FieldRow>
        </div>
      )}
      ...
      <div className="qa-auth-body">
        ...
        {a.type === 'bearer' && (
          <FieldRow label={t('request.auth.token')} hint={t('request.auth.tokenHint')}>
            <SecretInput value={a.bearer} onChange={(v: string) => set('bearer', v)} placeholder="eyJhbGciOiJIUzI1NiI..." />
          </FieldRow>
        )}
        ...
        {a.type === 'apiKey' && (
          <>
            ...
            <FieldRow label={t('common.value')}><SecretInput value={a.apiKey.value}
              onChange={(v: string) => set('apiKey', { ...a.apiKey, value: v })} placeholder="sk_live_..." /></FieldRow>
        ...
        {a.type === 'basic' && (
          <>
            ...
            <FieldRow label={t('request.auth.password')}><SecretInput value={a.basic.pass}
              onChange={(v: string) => set('basic', { ...a.basic, pass: v })} placeholder="password" /></FieldRow>
          </>
        )}
        {a.type === 'aws' && (
          <>
            <FieldRow label={t('request.auth.credentialProfile')}>
              <Dropdown value={a.aws.profile}
                        options={window.QA.CRED_PROFILES.filter((p: any) => p.type === 'aws').map((p: any) => p.name)}
