...
export function loadPrivacyCfg(): PrivacyCfg {
  const raw = loadVersioned<Record<string, unknown>>(
    'qa_ai_privacy',
    PRIVACY_CFG_VERSION,
    PRIVACY_MIGRATIONS,
    {}
...
const CLOUD_HOSTS =
  /(^|\.)(openai\.com|anthropic\.com|azure\.com|googleapis\.com|cohere\.ai|amazonaws\.com|mistral\.ai)$/i;

export function classifyDestination(
  cfg: { provider?: string; baseUrl?: string } | null | undefined
): AiDestination {
  ...
  let host = '';
  try {
    host = new URL(cfg!.baseUrl!).hostname.replace(/^\[|\]$/g, '');
  } catch {
    host = '';
  }
...
export function resolvePolicy(
  cfg: Partial<PrivacyCfg> | null | undefined,
...
const RE_TOKEN_PREFIX =
  /\b(?:gh[pousr]_[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{10,}|sk-[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16})\b/g;
const RE_SECRET =
  /\beyJ[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{4,}\b|\b[A-Za-z0-9_-]{40,}\b/g;
...
const KEY_TOKEN =
  /\b(?:gh[pousr]_[A-Za-z0-9]{20,}|xox[baprs]-[A-Za-z0-9-]{10,}|sk-[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16})\b/;
...
export function buildScrubber(cfg: Partial<PrivacyCfg> | null | undefined): Scrubber {
  ...
  const denyRe = new RegExp(
    '\\b(' +
      denyNames.map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|') +
      ')\\b(\\s*[=:]\\s*)("[^"]*"|\\S+)',
    'gi'
  );
  const custom: RegExp[] = [];
...
export function redactUrlForAI(url: unknown): string {
  try {
    ...
    const segs = pathPart
      .split('/')
      .map((seg) => (seg && (ID_SEG.test(seg) || SECRET_SEG.test(seg)) ? '{id}' : seg))
      .join('/');
...
function bodyStrFor(redacted: RedactedBody | null | undefined): string {
  ...
  if (redacted.nonJson)
    return `(non-JSON body: ${redacted.nonJson.contentType || 'unknown'}, ${redacted.nonJson.byteLength} bytes)`;
...
export const AI_KINDS: Record<string, AiKind> = {
    ...
    redact(p, ctx) {
      if (ctx.mode === 'full') {
        return {
          ...
          time: p.time,
          expected: p.expected || [],
          bodyText: p.body == null ? '(no body)' : JSON.stringify(p.body).slice(0, 1500),
        };
      }
      return {
        method: p.method,
        url: redactUrlForAI(p.url),
        status: p.status,
        statusText: p.statusText,
        time: p.time,
        expected: p.expected || [],
        bodyText: bodyStrFor(redactBody(p.body, p.headers)),
      };
    },
    buildPrompt(r) {
      return [
        ...
        r.expected.length
          ? `EXPECTED (assertions): ${r.expected.join('; ')}`
  ...
  testgen: {
    redact(p, ctx) {
      ...
      const red =
        p.source === 'openapi' ? redactOpenApi(p.input) : redactText(p.input, ctx.scrubber);
