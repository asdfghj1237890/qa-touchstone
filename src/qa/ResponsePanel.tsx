...
function reqReportObj(req: QaRequest, env: QaEnv, response: any) {
  const headers = req.headers.filter(h => h.on && h.key).reduce((o, h) => (o[h.key] = h.value, o), {} as Record<string, string>);
  const params = req.params.filter(p => p.on && p.key).reduce((o, p) => (o[p.key] = p.value, o), {} as Record<string, string>);
  ...
  } as Record<string, any>)[req.auth.type];
  return {
...
function reqReportHtml(req: QaRequest, env: QaEnv, response: any, t: I18nValue['t']) {
  ...
  const hrows = (h: Record<string, unknown> | null | undefined) => Object.entries(h || {}).map(([k, v]) => `<tr><td class="k">${escHtml(k)}</td><td>${escHtml(v)}</td></tr>`).join('') || `<tr><td colspan="2" class="muted">${escHtml(t('common.none'))}</td></tr>`;
...
function ResponsePanel({ state, response, req, env, testList }: ResponsePanelProps) {
  ...
  const reqPreview = {
    ...
    auth: req.auth.type,
    headers: req.headers.filter(h => h.on && h.key).reduce((o, h) => (o[h.key] = h.value, o), {} as Record<string, string>),
      ...
      {state === 'done' && (
        <>
          <div className="qa-tabs qa-tabs--resp">
            ...
            ] as Array<{ key: string; label: string; icon: string; count?: number | string }>).map(t => (
