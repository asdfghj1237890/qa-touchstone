...
function SuiteRunBar({ suite, onRun, onStop, canExport, onExport, canEvidence, onSaveEvidence }: SuiteRunBarProps) {
  const { t } = useI18n();
              ...
              {expOpen && (
                <span className="qa-report-menu">
                  <label className="qa-report-redaction">{t('report.redaction')}:
                    <select value={(!canEvidence && redaction === 'evidence') ? 'redacted' : redaction} onChange={e => setRedaction(e.target.value as ReportRedaction)}>
