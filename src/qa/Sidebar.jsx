...
function histDownload(name, content, mime) {
...
function histEntry(h) {
  ...
  const meta = all.find(r => r.id === h.id);
  return {
...
function exportHistory(items, fmt) {
  ...
  return (
    <nav className="qa-rail">
      ...
      <div className="qa-rail-foot">
        <div className="qa-rail-ver">0.18</div>
      </div>
    </nav>
  );
}

function CollectionRow({ req, active, onClick }) {
  return (
...
function CollectionsPanel({ selectedReq, onSelectRequest, env, setEnv, history, onSelectHistory, onImport, dataVersion }) {
  ...
  const toggleHSel = (i) => setHSel(s => s.includes(i) ? s.filter(x => x !== i) : [...s, i]);
