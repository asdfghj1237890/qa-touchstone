...
function qaEffectiveRequest(
  ...
  (req.headers || [])
    .filter((h) => h.on && h.key)
    .forEach((h) => headers.push([sub(h.key), sub(h.value)]));
  ...
  if (a.type === 'bearer') headers.push(['Authorization', 'Bearer ' + sub(a.bearer)]);
    ...
    headers.push(
      ['X-Amz-Date', '<computed>'],
      ['Authorization', 'AWS4-HMAC-SHA256 …<signed by client>']
    );
  ...
  if (req.bodyMode === 'json' && req.body) {
    ...
    body = JSON.stringify(
      { query: sub(req.gqlQuery || ''), variables: safeJson(sub(req.gqlVars || '')) || {} },
      null,
      2
    );
    ctype = 'application/json';
  ...
  if (ctype && !headers.some(([k]) => k.toLowerCase() === 'content-type'))
...
function genPython(e: QaEffectiveReq) {
  ...
  out.push(
    ``,
    `resp = requests.request(${qPy(e.method)}, url${e.headers.length ? ', headers=headers' : ''}${dataArg})`,
      ...
      <div className="qa-modal qa-codegen" onMouseDown={(ev) => ev.stopPropagation()}>
        <div className="qa-modal-head">
          <span className="qa-modal-title">
            <Icon name="code" size={15} /> {t('codegen.title')}
          </span>
          <div className="qa-modal-head-route">
            <MethodBadge method={req.method} size="sm" /> <span>{e.url}</span>
          </div>
        ...
        <div className="qa-codegen-bar">
          ...
          <label className="qa-codegen-resolve">
            <MiniCheck on={resolve} onClick={() => setResolve((r) => !r)} />{' '}
            {t('codegen.resolveVars')}
          </label>
          <button className="qa-hist-expbtn" onClick={copy}>
