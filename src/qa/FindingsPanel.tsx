...
import type {
  FindingStatus, LifecycleRecord, LifecycleState, Presence, Severity, SnapshotItem, SnapshotsState, UnionFinding,
} from './types';
...
function FindingsPanel({ union = [], snapshots: snapshotsProp, lifecycle: lifecycleProp, onPinBaseline, scopeMismatch = false }: FindingsPanelProps) {
  const { t } = useI18n();
  ...
  const patch = useCallback((fp: string, p: Partial<LifecycleRecord>) => {
                  ...
                  <tr className="qa-find-detail"><td colSpan={5}>
                    ...
                    <label className="qa-find-ctl">{t('findings.severityOverride')}:
                      <select aria-label={t('findings.severityOverride')} value={(r.record && r.record.severityOverride) || ''}
                              onChange={e => patch(r.fp, { severityOverride: (e.target.value || null) as Severity | null })}>
