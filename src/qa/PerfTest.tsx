...
const pfCollectionOptions = (): Array<{ value: string; label: string; title: string }> => window.QA.COLLECTIONS.map((c: any) => ({
  value: c.id,
  label: c.name || 'Collection',
  title: c.name || c.id,
}));

const pfTargets = (collectionId: string): PerfTarget[] => window.QA.COLLECTIONS.filter((c: any) => !collectionId || c.id === collectionId).flatMap((c: any) =>
  c.folders.flatMap((f: any) => f.requests.map((r: any) => {
...
function Chart({ pts, max, color, label, unit, dur, emptyLabel = 'No data yet' }: ChartProps) {
  const [hover, setHover] = usePF<{ i: number; leftPct: number; topPct: number; t: number; v: number } | null>(null); // { i, leftPct, topPct, t, v } | null
  const n = pts.length;
...
function Stat({ label, value, unit, accent, delta, better }: StatProps) {
  ...
  return (
    <div className="pf-stat">
      ...
      <div className="pf-stat-value" style={accent ? { color: 'var(--accent)' } : (null as any)}>{value}<em>{unit}</em></div>
...
function PerfTest({ env, vars, onRunningChange }: PerfTestProps) {
  const { t } = useI18n();
  const [targetCollection, setTargetCollection] = usePF<string>(() => window.QA.COLLECTIONS[0]?.id || '');
  ...
  const pickType = (t: PerfType) => { setType(t); setStages(clone(TYPE_META[t].stages)); setSlo({ ...TYPE_META[t].slo }); };

  const setStage = (i: number, k: 'd' | 't', v: number) => setStages(s => s.map((st, idx) => idx === i ? { ...st, [k]: Math.max(0, +v || 0) } : st));
  const addStage = () => setStages(s => [...s, { d: 10, t: 50 }]);
  const delStage = (i: number) => setStages(s => s.length > 1 ? s.filter((_, idx) => idx !== i) : s);

  const toggleSel = (i: number) => setSelected(s => s.includes(i) ? s.filter(x => x !== i) : [...s, i]);
  ...
  const run = async () => {
    ...
    const reqEntry = window.QA.COLLECTIONS
      .flatMap((c: any) => c.folders.flatMap((f: any) => f.requests))
    ...
    const sub = (t: string | null | undefined) => (window.qaSubstitute ? window.qaSubstitute(t || '', activeMap) : (t || ''));
    ...
    const qParts = (det.params || [])
      ...
      .map((p: any) => `${encodeURIComponent(sub(p.key))}=${encodeURIComponent(sub(p.value || ''))}`);
    ...
    const session: PerfSession = { state, scriptPath, stopped: false, buffer: '' };
    acc.current = session;
  ...
  const d = (k: keyof K6Metrics) => (baseline && shown && !running) ? shown.m[k] - baseline.m[k] : null;
      ...
      <div className="pf-config">
        ...
        <div className="pf-types">
          {Object.entries(TYPE_META).map(([k, meta]) => (
            <button key={k} className="pf-type" data-active={type === k ? '1' : '0'} onClick={() => !running && pickType(k as PerfType)} disabled={running}>
