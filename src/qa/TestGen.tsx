...
type CaseType = 'happy' | 'edge' | 'negative';
type SourceKind = 'bdd' | 'openapi' | 'text';

/** 產生的一筆測試案例（heuristic / LLM 兩條路共用）。 */
interface TestCase {
  id: number;
  title: string;
  steps: string[];
...
function loadPdfjs() {
  if (!_pdfjsPromise) {
    _pdfjsPromise = import('pdfjs-dist').then((mod: any) => {
...
function classify(text: string): CaseType {
  ...
  if (
    /invalid|unauthor|missing|without|forbidden|expired|wrong|reject|fail|denied|not found|409|40\d|50\d/.test(
      t
    )
  )
    return 'negative';
  if (/empty|maximum|minimum|boundary|limit|too |duplicate|special|long|zero|large/.test(t))
    return 'edge';
  return 'happy';
}

function inferStatus(text: string): number {
  const m =
...
function inferMethodPath(steps: string[], title: string): { method: string; path: string } {
...
function parseBDD(src: string): TestCase[] {
  const lines = src.split('\n');
  const cases: Array<{ title: string; steps: string[] }> = [];
  ...
  return cases.map((c, i) => {
    ...
    const thenLine = c.steps.find(
      (s) => /^Then|^And/i.test(s) && /(contain|include|should|error|body|array|id)/i.test(s)
    );
    if (thenLine) {
    ...
    return {
      ...
      status,
      assertions,
      type: classify(c.title + ' ' + c.steps.join(' ')),
    };
  });
}

function parseOpenAPI(src: string): TestCase[] | null {
  let spec: any;
  try {
    spec = JSON.parse(src);
  } catch {
  ...
  Object.entries(spec.paths).forEach(([path, ops]) => {
    Object.entries(ops as Record<string, any>).forEach(([method, op]) => {
      const M = method.toUpperCase();
      const okStatus = M === 'POST' ? 201 : M === 'DELETE' ? 204 : 200;
      cases.push({
        id: i++,
        title: (op.summary || `${M} ${path}`) + ' — happy path',
        ...
        assertions: [
          ...
          M === 'GET' ? 'response.body matches schema' : 'response.body has "id"',
        ],
      });
      if (M === 'POST') {
        cases.push({
          id: i++,
          title: `${op.summary || path} — missing required field`,
          steps: [
            `When I POST ${path} without a required field`,
            'Then the response status should be 400',
          ],
          method: M,
          path,
          status: 400,
          type: 'negative',
          assertions: ['response.status == 400', 'response.body has "error"'],
        });
      }
      if (path.includes('/me') || path.includes('{id}')) {
        cases.push({
          id: i++,
          title: `${op.summary || path} — unauthenticated`,
          steps: [
...
function splitReqWhenThen(s: string): string[] {
  const m = s.match(
...
function parseText(src: string): TestCase[] {
  const cleaned = String(src).replace(/\r/g, '');
  const raw = cleaned.split(/\n|(?<=[.;])\s+(?=[A-Z0-9"])/);
  const REQ =
    /\b(must|should|shall|will|needs? to|is required|are required|cannot|can't|only|reject|returns?|receives?|respond|support)\b/i;
...
const TYPE_CHIP: Record<CaseType, { labelKey: string; color: string }> = {
...
function extractCases(raw: unknown): TestCase[] | null {
  ...
  if (close > 0) {
    try {
      arr = JSON.parse(txt.slice(0, close + 1));
    } catch {
      arr = null;
    }
  }
  if (!Array.isArray(arr)) {
    ...
    for (let i = 1; i < txt.length; i++) {
      ...
      if (ch === '{') {
        ...
        if (depth === 0 && start >= 0) {
          try {
            arr.push(JSON.parse(txt.slice(start, i + 1)));
          } catch {}
          start = -1;
        }
      }
  ...
  return arr.slice(0, 12).map((c: any, i: number) => ({
    ...
    assertions: Array.isArray(c.assertions)
      ? c.assertions.map(String).slice(0, 5)
      : [`response.status == ${parseInt(c.status, 10) || 200}`],
    draft: !c.path || c.path === '—',
  }));
}
...
function CaseCard({ c, index, onAdd }: CaseCardProps) {
  const { t } = useI18n();
  ...
  return (
    <div className="tg-case qa-anim-in" style={{ animationDelay: `${Math.min(index, 8) * 45}ms` }}>
      <button className="tg-case-head" onClick={() => setOpen((o) => !o)}>
        ...
        <span className="tg-case-status" style={{ color: window.QATheme.statusColor(c.status) }}>
          {c.status}
        </span>
      ...
      {open && (
        <div className="tg-case-body">
          <div className="tg-path">
            ...
            {c.draft && (
              <span className="tg-draft">
                <Icon name="link" size={11} /> {t('testgen.mapEndpoint')}
              </span>
            )}
          </div>
            ...
            {c.steps.map((s, i) => {
              ...
              return (
                <div key={i} className="tg-step">
                  <span className="tg-step-kw">{kw}</span>
                  <span>{s.replace(/^(Given|When|Then|And|But)\s*/i, '')}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="tg-asserts">
            {c.assertions.map((a, i) => (
              <div key={i} className="tg-assert">
                <Icon name="check" size={12} /> <code>{a}</code>
              </div>
            ))}
          </div>
          <button className="tg-addone" onClick={() => onAdd && onAdd(c)}>
            <Icon name="plus" size={12} /> {t('testgen.addOne')}
          </button>
        </div>
      )}
    </div>
...
export interface TestGenProps {
  openSettings?: (tab: string) => void;
  onAddTests?: (method: string, path: string, assertions: any[]) => boolean;
}

function TestGen({ openSettings, onAddTests }: TestGenProps) {
  const { t } = useI18n();
  const [source, setSource] = useTG<SourceKind>('bdd');
  const [toast, setToast] = useTG('');
  const caseAsserts = (c: TestCase) => {
    const a: any[] = (c.assertions || [])
      .map(window.qaParseAssertion!)
      .filter((x) => x.type !== 'info');
    if (!a.some((x) => x.type === 'status'))
      a.unshift({ type: 'status', op: 'eq', value: c.status, on: true });
    return a;
  };
  const addAll = () => {
    let n = 0;
    cases.forEach((c) => {
      if (onAddTests && onAddTests(c.method, c.path, caseAsserts(c))) n++;
    });
    setToast(
      n
        ? t('testgen.toast.addedAll', {
            count: n,
            requestLabel: t(n === 1 ? 'testgen.toast.requestOne' : 'testgen.toast.requestMany'),
          })
        : t('testgen.toast.noMatches')
    );
    setTimeout(() => setToast(''), 2600);
  };
  const addOne = (c: TestCase) => {
    const ok = onAddTests && onAddTests(c.method, c.path, caseAsserts(c));
    setToast(
      ok
        ? t('testgen.toast.addedOne', {
            count: caseAsserts(c).length,
            method: c.method,
            path: c.path,
          })
        : t('testgen.toast.noMatch', { method: c.method, path: c.path })
    );
    setTimeout(() => setToast(''), 2600);
  };
  const [input, setInput] = useTG(SAMPLE_BDD);
  const [cases, setCases] = useTG<TestCase[]>([]);
  const [busy, setBusy] = useTG(false);
  const [err, setErr] = useTG('');
  const [extracting, setExtracting] = useTG('');
  const fileRef = React.useRef<HTMLInputElement | null>(null);
  const [engine, setEngine] = useTG<'ai' | 'heuristic'>(() =>
    window.claude && (window.claude as any).complete ? 'ai' : 'heuristic'
  );
  ...
  const onFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (name.endsWith('.pdf')) {
        ...
        setSource('text');
        setInput(text.trim() || t('testgen.err.noExtract'));
        setExtracting('');
      } else {
      ...
      setExtracting('');
      setErr(t('testgen.err.readFile', { message: ex.message }));
    }
  };
      ...
      const result =
        source === 'bdd'
          ...
          : source === 'openapi'
            ...
            : parseText(input);
      if (result === null) {
        setErr(t('testgen.err.invalidJson'));
        setBusy(false);
        return;
      }
      if (result.length === 0) {
        setErr(t('testgen.err.noStatements'));
        setBusy(false);
        return;
      }
      setCases(result);
  ...
  const generate = async () => {
    ...
    if (cfg.provider !== 'builtin' && !cfg.key.trim()) {
      setErr(t('testgen.err.noKey'));
      return;
    }
    if (cfg.provider === 'builtin' && !(window.claude && (window.claude as any).complete)) {
      setErr(t('testgen.err.builtinUnavailable'));
      return;
    }
    setBusy(true);
    try {
      ...
      if (!parsed || !parsed.length) {
        setErr(t('testgen.err.badModelOutput'));
        setBusy(false);
        return;
      }
      setCases(parsed);
      ...
      if (ex && ex.name === 'AiCancelledError') {
        setBusy(false);
        return;
      }
      setErr(t('testgen.err.llmFailed', { message: ex.message }));
      setBusy(false);
    }
  };
  ...
  const counts = cases.reduce<Record<string, number>>(
    (o, c) => ((o[c.type] = (o[c.type] || 0) + 1), o),
    {}
  );
  const endpoints = new Set(cases.filter((c) => c.path !== '—').map((c) => `${c.method} ${c.path}`))
    .size;
    ...
    <div className="tg">
      <div className="tg-source">
        ...
        <div className="qa-segs qa-segs--mini tg-srctabs">
          <button data-active={source === 'bdd' ? '1' : '0'} onClick={() => switchSource('bdd')}>
            {t('testgen.source.bdd')}
          </button>
          <button
            data-active={source === 'openapi' ? '1' : '0'}
            onClick={() => switchSource('openapi')}
          >
            {t('testgen.source.openapi')}
          </button>
          <button data-active={source === 'text' ? '1' : '0'} onClick={() => switchSource('text')}>
            {t('testgen.source.text')}
          </button>
        </div>
        <div className="tg-editor-label">
          <span>
            <Icon name="fileText" size={13} />{' '}
            {extracting ? t('testgen.extracting', { file: extracting }) : fileName}
          </span>
          <button className="qa-link" onClick={() => fileRef.current && fileRef.current.click()}>
            <Icon name="upload" size={12} /> {t('testgen.upload')}
          </button>
          <input
            ref={fileRef}
            type="file"
            accept=".txt,.md,.feature,.json,.pdf"
            style={{ display: 'none' }}
            onChange={onFile}
          />
        </div>
        ...
        <div className="tg-hint">
          ...
          <span>{t('testgen.hint')}</span>
        </div>

        <div className="tg-engine">
          <div className="qa-segs qa-segs--mini">
            <button data-active={engine === 'ai' ? '1' : '0'} onClick={() => setEngine('ai')}>
              <Icon name="sparkle" size={13} /> {t('testgen.engine.ai')}
            </button>
            <button
              data-active={engine === 'heuristic' ? '1' : '0'}
              onClick={() => setEngine('heuristic')}
            >
              <Icon name="code" size={13} /> {t('testgen.engine.heuristic')}
            </button>
          </div>
          {engine === 'ai' && (
            <span className="tg-engine-model">
              {modelLabel}
              {aiBuiltin ? ` · ${t('testgen.noKey')}` : ''}
            </span>
          )}
          <button
            className="qa-link tg-cfg-toggle"
            onClick={() => openSettings && openSettings('llm')}
          >
            <Icon name="settings" size={12} /> {t('testgen.settings')}
          </button>
        </div>
      ...
      <div className="tg-output">
        {cases.length > 0 && (
          <div className="tg-summary qa-fade">
            ...
            <div className="tg-sum-chips">
              {Object.entries(TYPE_CHIP).map(([k, v]) =>
                counts[k] ? (
                  <span
                    key={k}
                    className="tg-chip"
                  ...
                  >
                    {counts[k]} {t(v.labelKey)}
                  </span>
                ) : null
              )}
              <button className="tg-addall" onClick={addAll}>
                <Icon name="plus" size={13} /> {t('testgen.addAll')}
              </button>
            </div>
          </div>
        )}
        {cases.length > 0 && drafts > 0 && (
          <div className="tg-note qa-fade">
            <Icon name="link" size={13} />{' '}
            {t('testgen.draftNote', {
              drafts,
              caseLabel: t(drafts > 1 ? 'testgen.caseMany' : 'testgen.caseOne'),
            })}
          </div>
        )}
              ...
              <div>
                {engine === 'ai'
                  ? t('testgen.busy.ai', {
                      ...
                      source: t(
                        source === 'bdd'
                          ? 'testgen.busy.source.feature'
                          : source === 'openapi'
                            ? 'testgen.busy.source.spec'
                            : 'testgen.busy.source.document'
                      ),
                    })
                  : t('testgen.busy.heuristic', {
                      source: t(
                        source === 'bdd'
                          ? 'testgen.busy.source.scenarios'
                          : source === 'openapi'
                            ? 'testgen.busy.source.operations'
                            : 'testgen.busy.source.requirements'
                      ),
                    })}
              </div>
            </div>
          ...
          {!busy && cases.length === 0 && !err && (
            <div className="tg-empty">
              ...
              <div className="tg-empty-sub">
                {t('testgen.empty.subBefore')} <kbd>{t('testgen.generate')}</kbd>
                {t('testgen.empty.subAfter')}
              </div>
            </div>
          )}
          {err && (
        ...
        {toast && (
          <div className="tg-toast qa-fade">
