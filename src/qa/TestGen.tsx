...
function TestGen({ openSettings, onAddTests }: TestGenProps) {
  ...
  const caseAsserts = (c: TestCase) => {
    const a: any[] = (c.assertions || []).map(window.qaParseAssertion!).filter(x => x.type !== 'info');
  ...
  const [engine, setEngine] = useTG<'ai' | 'heuristic'>(() => (window.claude && (window.claude as any).complete) ? 'ai' : 'heuristic');
  const cfg = window.loadLlmCfg!();

  const SAMPLES: Record<SourceKind, string> = { bdd: SAMPLE_BDD, openapi: SAMPLE_OPENAPI, text: SAMPLE_TEXT };
  const switchSource = (s: SourceKind) => { setSource(s); setInput(SAMPLES[s]); setCases([]); setErr(''); };
  ...
  const counts = cases.reduce<Record<string, number>>((o, c) => (o[c.type] = (o[c.type] || 0) + 1, o), {});
