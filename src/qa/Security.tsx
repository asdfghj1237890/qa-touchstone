...
import type {
  BolaAttackCell, BolaConfig, BolaRefCell, BolaResults, Contract, EvidenceArtifact, Expectation,
  Finding, Identity, MatrixCell, MatrixResults, RateLimitConfig, RateLimitTest, ReportRedaction,
  Snapshot, UnionFinding, Verdict,
} from './types';

const { useState: useS, useEffect: useE, useMemo, useRef, useCallback } = React;
const EXPECTS: Expectation[] = ['allow', 'deny', 'skip'];
const VERDICT_LABEL: Record<Verdict, string> = { pass: 'security.verdict.pass', fail: 'security.verdict.fail', vuln: 'security.verdict.vuln', inconclusive: 'security.verdict.inconclusive' };
...
let idSeq = 1;
function newIdentity(): SecIdentity { return { id: `id_${Date.now()}_${idSeq++}`, name: '', auth: blankAuth() }; }
...
function allRequests(): ReqOption[] {
  return (window.QA.COLLECTIONS || []).flatMap((c: any) =>
    (c.folders || []).flatMap((f: any) => (f.requests || []).map((r: any) => ({ reqId: r.id, method: r.method, path: r.path, name: r.name, folder: f.name }))));
}
...
function IdentityEditor({ identity, onChange, onClose, env, vars, sslVerify }: IdentityEditorProps) {
  const { t } = useI18n();
...
function SecurityPage({ env = { label: 'None', baseUrl: '' }, vars, cookies = [], sslVerify = true }: SecurityPageProps) {
  const { t } = useI18n();
  const [identities, setIdentities] = useS<SecIdentity[]>(() => {
    const cfg = loadMatrixConfig();
    return ((cfg && cfg.identities && cfg.identities.length) ? cfg.identities : [anonIdentity()]) as SecIdentity[];
  });
  const [endpoints, setEndpoints] = useS<SecEndpoint[]>(() => { const cfg = loadMatrixConfig(); return (cfg && cfg.endpoints) || []; });
  const [expect, setExpect] = useS(() => { const cfg = loadMatrixConfig(); return (cfg && cfg.expect) || {}; });
  const [denySet, setDenySet] = useS<number[]>(() => { const cfg = loadMatrixConfig(); return (cfg && cfg.denySet) || DEFAULT_DENY_SET; });
  ...
  const [aiScan, setAiScan] = useS<{ busy: boolean; error: string | null }>({ busy: false, error: null });
  const [mode, setMode] = useS('matrix');
  const [bola, setBola] = useS<BolaConfig>(() => { const cfg = loadMatrixConfig(); return (cfg && cfg.bola) || { tests: [] }; });
  const [bolaResults, setBolaResults] = useS<BolaResults>({});
  const [rateLimit, setRateLimit] = useS<RateLimitConfig>(() => { const cfg = loadMatrixConfig(); return (cfg && cfg.rateLimit) || { tests: [] }; });
  const [rlResults, setRlResults] = useS<RateLimitResultsMap>({});
  const [snapshots, setSnapshots] = useS(() => loadSnapshots());
  const [suite, setSuite] = useS<SuiteUiState>({ running: false, engine: null, done: 0, total: 0, lastRecord: null });
  ...
  const onBolaFindings = useCallback((list: UnionFinding[]) => setBolaFindings(list), []);
  const onRlFindings = useCallback((list: UnionFinding[]) => setRlFindings(list), []);
  ...
  const removeIdentity = (id: string) => {
    ...
    setResults(r => Object.fromEntries(Object.entries(r).map(([rid, row]) => {
      const { [id]: _gone, ...rest } = row; return [rid, rest] as const;
    })));
  };
  ...
  const togglePriv = (reqId: string, val: boolean) => setEndpoints(xs => xs.map(e => e.reqId === reqId ? { ...e, privileged: val } : e));

  const runner = (ep: SecEndpoint, identity: Identity) => qaRunSavedRequest({ id: ep.reqId }, {
  ...
  const matrixCellFindings = (reqId: string, cell: MatrixCell, baselines: Record<string, Contract>) => {
  ...
  const bolaRunner: BolaRunner = (test, identity, idValue) => qaRunSavedRequest({ id: test.reqId as string }, {
    env, vars, cookies: cookies as QaRunCookie[], sslVerify, authOverride: identity.auth as any, oauthToken: identity._oauthToken as any,
  ...
  const runRlTest = async (
    ...
    setRes(r => ({ ...r, [test.id]: { progress: { done: 0, n: test.n as number }, stats: null, verdict: null } }));
  ...
  const runFullSuite = async () => {
    ...
    const bolaAdapter = async (cfg: any, { signal }: { signal?: AbortSignal | null }) => {
      ...
      await runBola({ identities: cfg.identities, tests: cfg.tests }, bolaRunner, {
        ...
        onCell: (testId, attackerId, ownerId, cell) => {
          ...
          if (attackerId == null) out[testId] = { ...tr, reference: { ...tr.reference, [ownerId]: cell as BolaRefCell } };
          else out[testId] = { ...tr, attacks: { ...tr.attacks, [attackerId]: { ...(tr.attacks[attackerId] || {}), [ownerId]: cell as BolaAttackCell } } };
          setBolaResults({ ...out });
        },
      });
      return out;
    ...
    if (rec.status === 'complete') {
      ...
      const record: Snapshot = { runId: rec.finishedAt, scopeHash, createdAt: rec.finishedAt,
                  ...
                  {identities.map(id => {
                    ...
                    return (
                      <td key={id.id} className={`qa-sec-cell qa-sec-cell--${v || 'none'}`}
                        ...
                        {cell && <span className="qa-sec-verdict">{cell.status ?? '—'} · {t(VERDICT_LABEL[v as Verdict] || 'security.cell.notRun')}</span>}
      ...
      {drawerCell && (
        <div className="qa-sec-drawer">
          <div className="qa-sec-drawer-head">
            <span>{drawerCell.status ?? '—'} · {t(VERDICT_LABEL[drawerCell.verdict as Verdict] || 'security.cell.notRun')}</span>
