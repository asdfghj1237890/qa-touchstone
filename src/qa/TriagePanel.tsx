// src/qa/TriagePanel.tsx
// QA Touchstone — batch AI triage card. Advisory only: renders a separate
...
export interface TriagePanelProps {
  union?: TriageFinding[];
  aiReady?: boolean;
  onGoToEngine?: (engine: string) => void;
  /** 測試注入用：覆寫預設的 runTriage。 */
  runner?: (union: TriageFinding[]) => Promise<TriageRunResult>;
}

function TriagePanel({ union = [], aiReady, onGoToEngine, runner }: TriagePanelProps) {
  const { t } = useI18n();
  const doTriage = runner || runTriage;
  const [open, setOpen] = useS(true);
  const [busy, setBusy] = useS(false);
  const [error, setError] = useS<string | null>(null);
  const [triage, setTriage] = useS<TriageRunResult | null>(null);
