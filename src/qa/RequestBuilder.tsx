...
/** KVTable 的列（params/headers 用 QaKVRow；local vars 的 on 可缺漏）。 */
...
export interface KVTableProps {
  rows: KVTableRow[];
  // 回傳列陣列維持呼叫端原始形狀（QaKVRow[] / QaLocalVar[]），故以 any[] 過渡。
...
function KVTable({ rows, onChange, keyPh = 'Key', valPh = 'Value', varMap }: KVTableProps) {
  const { t } = useI18n();
    ...
    <div className="qa-kv">
      ...
      {rows.map((r, i) => (
        <div className="qa-kv-rowwrap" key={i}>
          ...
          {varMap && r.on && window.qaHasVars!(r.value) && (
            <VarHint text={r.value} varMap={varMap} />
          )}
        </div>
      ))}
...
function VarHint({ text, varMap }: VarHintProps) {
  ...
  return (
    <div className="qa-varhint" data-warn={unknown.length ? '1' : '0'}>
      ...
      {unknown.length ? (
        <span>
          {t('request.var.unresolved', { vars: unknown.map((u) => `{{${u}}}`).join(', ') })}
        </span>
      ) : (
        <span>{resolved}</span>
      )}
...
function newAssert(type: string): QaAssertion {
  switch (type) {
    case 'time':
      return { type, op: 'lt', value: 500, on: true };
    case 'bodyHas':
      return { type, path: 'data', on: true };
    case 'bodyArray':
      return { type, on: true };
    case 'header':
      return { type, name: 'content-type', op: 'exists', value: '', on: true };
    default:
      return { type: 'status', op: 'eq', value: 200, on: true };
  }
}
export interface AssertionsEditorProps {
  rows: QaAssertion[];
  onChange: (rows: QaAssertion[]) => void;
}

function AssertionsEditor({ rows, onChange }: AssertionsEditorProps) {
  const { t } = useI18n();
  const upd = (i: number, d: Record<string, unknown>) =>
    onChange(rows.map((r, idx) => (idx === i ? { ...r, ...d } : r)));
  ...
  return (
    <div className="qa-asserts-ed">
      <div className="qa-asserts-hint">
        <Icon name="check" size={12} /> {t('request.assert.hint')}
      </div>
      {rows.length === 0 && (
        <div className="qa-auth-none">
          <Icon name="check" size={16} /> {t('request.assert.empty')}
        </div>
      )}
      {rows.map((a: any, i) => (
        <div className="qa-assert-row" key={i}>
          ...
          <div className="qa-assert-fields">
            <Dropdown
              ...
              onChange={(nextType: string) =>
                onChange(rows.map((r, idx) => (idx === i ? newAssert(nextType) : r)))
              }
            />
            {a.type === 'status' && (
              <>
                <Dropdown
                  ...
                  onChange={(op: string) => upd(i, { op })}
                />
                <input
                  className="qa-inp qa-assert-v"
                  type="number"
                  value={a.value}
                  onChange={(e) => upd(i, { value: +e.target.value || 0 })}
                />
              </>
            )}
            {a.type === 'time' && (
            ...
            {a.type === 'header' && (
              <>
                ...
                <Dropdown
                  value={a.op}
                  options={[
                    { value: 'exists', label: t('request.assert.exists') },
                    { value: 'eq', label: '=' },
                    { value: 'contains', label: t('request.assert.contains') },
                  ]}
                  onChange={(op: string) => upd(i, { op })}
                />
                {a.op !== 'exists' && (
          ...
          <button className="qa-kv-del" onClick={() => del(i)} aria-label={t('common.removeRow')}>
            <Icon name="x" size={12} />
          </button>
        </div>
      ))}
      <button className="qa-kv-add" onClick={() => onChange([...rows, newAssert('status')])}>
        <Icon name="plus" size={13} /> {t('request.assert.add')}
      </button>
    </div>
  );
}
...
function cookieHostLabel(
  env: QaEnv | null | undefined,
  ...
  try {
    ...
    try {
      ...
      return t('request.options.host.configured');
    }
  }
}
...
export interface OptionsEditorProps {
  env: QaEnv;
  sslVerify: boolean;
  setSslVerify: (on: boolean) => void;
  localVars?: QaLocalVar[] | null;
  setLocalVars: (list: QaLocalVar[]) => void;
  varMap?: Record<string, string> | null;
  cookies?: QaCookie[] | null;
  onOpenSettings?: (section: string) => void;
  /** 呼叫端額外傳入但元件未使用（保持 call site 原樣）。 */
  req?: QaRequest;
...
function OptionsEditor({
  ...
  return (
    <div className="qa-opts">
      ...
      <div className="qa-opt-block">
        <div className="qa-opt-row">
          <div className="qa-opt-text">
            <strong>
              <Icon name="shield" size={13} /> {t('request.options.ssl')}
            </strong>
        ...
        {!sslVerify && (
          <div className="qa-opt-warn">
            <Icon name="shield" size={13} /> {t('request.options.sslWarn')}
          </div>
        )}
      </div>
      ...
      {/* Local variables */}
      <div className="qa-opt-block">
        <div className="qa-opt-head">
          <span>
            <Icon name="code" size={13} /> {t('request.options.localVars')}
          </span>
          <span className="qa-scope-tag" data-scope="local">
            {t('request.options.highestPrecedence')}
          </span>
        </div>
        <p className="qa-opt-sub">
          {t('request.options.localVarsHint')} <code>{'{{name}}'}</code>.
        </p>
        <KVTable
          rows={localVars || []}
          onChange={setLocalVars}
          keyPh={t('common.variable')}
          valPh={t('common.value')}
          varMap={varMap}
        />
      </div>
      ...
      <div className="qa-opt-block">
        <div className="qa-opt-head">
          <span>
            <Icon name="globe" size={13} /> {t('request.options.cookiesFor', { host: hostLabel })}
          </span>
          <button className="qa-link" onClick={() => onOpenSettings && onOpenSettings('cookies')}>
            {t('request.options.manageJar')}
          </button>
        </div>
        {cookies && cookies.length ? (
          <div className="qa-ckmini">
            {cookies.map((c) => (
              <div className="qa-ckmini-row" key={c.id as any}>
                <span className="qa-ckmini-name">{c.name}</span>
                <span className="qa-ckmini-val">{c.value}</span>
                <span className="qa-ckmini-flags">
                  {c.secure ? 'Secure ' : ''}
                  {c.httpOnly ? 'HttpOnly' : ''}
                </span>
              </div>
            ))}
            <div className="qa-opt-sub" style={{ marginTop: 4 }}>
              {t('request.options.cookieSent')}
            </div>
          </div>
        ) : (
          <div className="qa-auth-none">
            <Icon name="globe" size={16} /> {t('request.options.noCookies')}
          </div>
        )}
      </div>
    </div>
...
function RequestBuilder({ onOpenSettings }: RequestBuilderProps) {
  const { t } = useI18n();
  const {
    env,
    sslVerify,
  ...
  const {
    ...
    collectionId,
    localList: localVars,
    setLocalForReq: setLocalVars,
    reqCookies: cookies,
    fetchOAuthToken,
  } = useRequest();
  const isLoading = respState === 'loading';
  const tests = allTests[req.id] || [];
  const setTests = (list: QaAssertion[]) => setAllTests((t2) => ({ ...t2, [req.id]: list }));
  ...
  const counts: Record<string, number | string> = {
    ...
    tests: (tests || []).length,
    options:
      (localVars || []).filter((v) => v.on !== false && v.key).length + (sslVerify ? 0 : 1) ||
      ((cookies || []).length ? '•' : 0),
  };
  const tabs = [
  ...
  return (
    <section className="qa-builder">
      ...
      <div className="qa-urlbar">
        <div className="qa-method">
          ...
          >
            ...
            <Icon
              name="chevron"
              size={13}
              className="qa-method-chev"
              data-open={methodOpen ? '1' : '0'}
            />
          </button>
          {methodOpen && (
            <div className="qa-method-menu">
              {METHODS.map((m) => (
                <button
                  ...
                  style={{ color: window.QATheme.methodColor(m) }}
                  data-active={m === req.method ? '1' : '0'}
                >
                  {m}
                </button>
              ))}
        ...
        {!sslVerify && (
          <button
            className="qa-iconbtn qa-ssl-off"
            title={t('request.action.sslOff')}
            onClick={() => setTab('options')}
          >
            <Icon name="shield" size={15} />
          </button>
        )}
        <button className="qa-iconbtn" title={t('request.action.save')}>
      ...
      <div className="qa-tabbody">
        ...
        {tab === 'headers' && (
          <KVTable
            ...
            keyPh={t('common.header')}
            valPh={t('common.value')}
            varMap={varMap}
          />
        )}
        {tab === 'auth' && (
          <AuthEditor req={req} patch={patch} oauthToken={oauthToken} onFetchOAuth={onFetchOAuth} />
        )}
        {tab === 'body' && (
          <div className="qa-body">
            <div className="qa-segs qa-segs--mini qa-body-modes">
              {(['none', 'json', 'graphql', 'form'] as const).map((m) => (
                <button
                  key={m}
                  data-active={req.bodyMode === m ? '1' : '0'}
                  onClick={() =>
                    patch(
                      m === 'graphql'
                        ? { bodyMode: m, method: req.method === 'GET' ? 'POST' : req.method }
                        : { bodyMode: m }
                    )
                  }
                >
            ...
            {req.bodyMode === 'none' && (
              <div className="qa-auth-none">
                <Icon name="code" size={16} /> {t('request.body.none')}
              </div>
            )}
              ...
              <>
                ...
                {window.qaHasVars!(req.body) &&
                  (window.qaUnknownVars!(req.body, varMap).length ? (
                    <div className="qa-varhint" data-warn="1">
                      <Icon name="x" size={11} />{' '}
                      <span>
                        {t('request.var.unresolved', {
                          vars: [...new Set(window.qaUnknownVars!(req.body, varMap))]
                            .map((u) => `{{${u}}}`)
                            .join(', '),
                        })}
                      </span>
                    ...
                    <div className="qa-varhint">
                      <Icon name="link" size={11} />{' '}
                      <span>
                        {t('request.var.resolved', {
                          vars: [...new Set(req.body.match(/\{\{[^}]+\}\}/g) || [])].join(', '),
                        })}
                      </span>
                    </div>
                  ))}
