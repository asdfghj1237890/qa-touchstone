...
import { useWorkspace, type QaEnv, type QaCookie, type QaLocalVar, type QaAssertion } from './state/WorkspaceContext';
...
export interface KVTableRow { key: string; value: string; on?: boolean }
...
function AssertionsEditor({ rows, onChange }: AssertionsEditorProps) {
  const { t } = useI18n();
  const upd = (i: number, d: Record<string, unknown>) => onChange(rows.map((r, idx) => idx === i ? { ...r, ...d } : r));
      ...
      {rows.map((a: any, i) => (
        <div className="qa-assert-row" key={i}>
          ...
          <div className="qa-assert-fields">
            <Dropdown value={a.type} options={assertOptions} onChange={(nextType: string) => onChange(rows.map((r, idx) => idx === i ? newAssert(nextType) : r))} />
            {a.type === 'status' && <>
              <Dropdown value={a.op} options={[{ value: 'eq', label: '=' }, { value: 'neq', label: '≠' }, { value: 'lt', label: '<' }, { value: 'gt', label: '>' }]} onChange={(op: string) => upd(i, { op })} />
              <input className="qa-inp qa-assert-v" type="number" value={a.value} onChange={e => upd(i, { value: +e.target.value || 0 })} />
            </>}
            {a.type === 'time' && <>
              <Dropdown value={a.op} options={[{ value: 'lt', label: '<' }, { value: 'gt', label: '>' }]} onChange={(op: string) => upd(i, { op })} />
              <input className="qa-inp qa-assert-v" type="number" value={a.value} onChange={e => upd(i, { value: +e.target.value || 0 })} />
            ...
            {a.type === 'header' && <>
              ...
              <Dropdown value={a.op} options={[{ value: 'exists', label: t('request.assert.exists') }, { value: 'eq', label: '=' }, { value: 'contains', label: t('request.assert.contains') }]} onChange={(op: string) => upd(i, { op })} />
...
function cookieHostLabel(env: QaEnv | null | undefined, varMap: Record<string, string> | null | undefined, t: I18nValue['t']) {
...
function OptionsEditor({ env, sslVerify, setSslVerify, localVars, setLocalVars, varMap, cookies, onOpenSettings }: OptionsEditorProps) {
  const { t } = useI18n();
