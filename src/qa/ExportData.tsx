...
function pmUrlObject(
  baseUrl: string,
  path: string,
  params: PmKVRowLike[] | null | undefined
): PmUrlObject {
  ...
  try {
    const u = new URL(
      raw.includes('://') ? raw : 'https://placeholder' + (path.startsWith('/') ? path : '/' + path)
    );
  ...
  const query = (params || [])
    .filter((p) => p.key)
    .map((p) => ({ key: p.key, value: p.value || '', disabled: p.on === false }));
...
function pmAuth(authType: string | undefined, _det: any) {
  switch (authType) {
    ...
    case 'aws':
      return { type: 'awsv4', awsv4: [{ key: 'service', value: 'execute-api' }] };
    default:
      return undefined;
  }
}
...
function buildPostmanCollection(col: any, env: any) {
  ...
  const toItem = (r: any) => {
    ...
    const headers = (det.headers || []).map((h: any) => ({
      key: h.key,
      value: h.value || '',
      disabled: h.on === false,
    }));
    ...
    if (det.graphql) {
      item.request.body = {
        mode: 'graphql',
        graphql: { query: det.graphql.query, variables: det.graphql.variables },
      };
    } else if (det.body) {
