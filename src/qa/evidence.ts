...
// emit raw on error.
import './setup';
import { redact } from './oracles';
import { fingerprint } from './findings';
import type { EvidenceArtifact, EvidenceResponse, FindingLike, RawEvidence, ScrubbedNode, SnapshotItem } from './types';
...
export function leafToken(v: unknown): string {
...
export function tokenizePath(p: string | null | undefined): string[] {
  const out: string[] = [];
  const re = /[^.[\]]+|\[(\d+)\]/g;
  let m;
...
// names, not secrets.
function scrub(node: unknown, markKey: string | null, depth: number, caps: { keys: number; truncated: boolean }): ScrubbedNode {
...
export function snippetAround(body: unknown, findingPath: string | null | undefined): { snippetPath: string; tree: ScrubbedNode; truncated: boolean } | null {
  try {
...
export function headerVal(headers: Record<string, unknown> | null | undefined, lowerName: string): unknown {
  try {
...
function asJson(body: unknown): { ok: true; value: unknown } | { ok: false } {
...
export function buildEvidenceArtifact(raw: RawEvidence | null | undefined, item: FindingLike | null | undefined): EvidenceArtifact | null {
  try {
    if (!raw) return null;
    ...
    if (resp) {
      const r: EvidenceResponse = { status: resp.status != null ? resp.status : null, headers: redactHeaders(resp.headers || {}), snippetPath: '', snippet: null, nonJson: null, truncated: false };
...
export function buildEvidenceMap(union: Array<FindingLike | null | undefined> | null | undefined): Map<string, EvidenceArtifact> {
  const map = new Map<string, EvidenceArtifact>();
...
export function embedEvidence(items: SnapshotItem[] | null | undefined, map: Map<string, EvidenceArtifact> | null | undefined): SnapshotItem[] {
