...
const SECRET_VALUE =
  /eyJ[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{4,}|AKIA[0-9A-Z]{16}|[A-Za-z0-9_-]{32,}/;
...
function scrub(
  ...
  markKey: string | null,
  depth: number,
  caps: { keys: number; truncated: boolean }
): ScrubbedNode {
  ...
  for (const k of keys) {
    ...
    if (isArr) (out as ScrubbedNode[]).push(child);
    else (out as { [k: string]: ScrubbedNode })[k] = child;
  }
  return out;
}
...
): { snippetPath: string; tree: ScrubbedNode; truncated: boolean } | null {
  try {
...
export function redactUrl(url: unknown): string {
  try {
    ...
    const segs = pathPart
      .split('/')
      .map((seg) => (seg && SECRET_VALUE.test(seg) ? REDACTED : seg))
      .join('/');
...
export function buildEvidenceArtifact(
  raw: RawEvidence | null | undefined,
  item: FindingLike | null | undefined
  ...
  try {
    ...
    if (resp) {
      const r: EvidenceResponse = {
        status: resp.status != null ? resp.status : null,
      ...
      if (s) {
        r.snippet = s.tree;
        r.snippetPath = s.snippetPath;
        r.truncated = s.truncated;
      } else if (!parsed.ok) {
        r.nonJson = {
          contentType: String(headerVal(resp.headers, 'content-type') || ''),
          length: typeof resp.body === 'string' ? resp.body.length : 0,
        };
      }
      artifact.response = r;
    }
...
export function buildEvidenceMap(
  union: Array<FindingLike | null | undefined> | null | undefined
...
export function embedEvidence(
  ...
  map: Map<string, EvidenceArtifact> | null | undefined
): SnapshotItem[] {
