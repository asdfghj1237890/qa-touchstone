...
export function worstSeverity(
  findings: Array<Pick<Finding, 'severity'>> | null | undefined
): Severity | null {
...
const RULES: SensitiveRule[] = [
  {
    ...
    value: /\beyJ[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{6,}\.[A-Za-z0-9_-]{4,}\b/,
  },
  {
    id: 'aws-key',
    group: 'secrets',
  ...
  },
  {
    ...
    group: 'secrets',
    severity: 'critical',
    title: 'Secret-named field present',
    key: /^(password|passwd|pwd|secret|client_secret|access_token|refresh_token|api_?key|token)$/i,
  },
  {
    id: 'email',
    group: 'pii',
  ...
  },
  {
    ...
    group: 'internal',
    severity: 'medium',
    title: 'Internal/debug field',
    key: /^(stack_?trace|exception|sql|query|internal.*|debug)$/i,
  },
];
...
export function scanSensitive(
  response: QaResponse | null | undefined,
...
export function inferContract(body: unknown): Contract {
  ...
  walkJson(body, (path, _key, value) => {
    contract[normalizePath(path)] = { type: jsType(value), required: true };
  });
  return contract;
}
...
export function checkSchema(
  ...
  if (body != null && typeof body === 'object') {
    walkJson(body, (path, _key, value) => {
      present[normalizePath(path)] = jsType(value);
    });
  }
...
export async function scanSensitiveLLM(
  ...
  try {
    raw = await doSend({
      site: 'sensitive-scan',
      kind: 'sensitive-scan',
      payload: { body: payload && payload.body, headers: (payload && payload.headers) || {} },
    });
  } catch (e: any) {
  ...
  let arr;
  try {
    arr = JSON.parse((String(raw).match(/\[[\s\S]*\]/) || ['[]'])[0]);
  } catch {
    return [];
  }
