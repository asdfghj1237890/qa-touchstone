...
function ImportModal({ onClose, onImport }: ImportModalProps) {
  ...
  const requestLabel = ok
    ? t(parsed.collection.count === 1 ? 'import.requestOne' : 'import.requestMany')
    : '';
  const folderLabel = ok
    ? t(parsed.collection.folders.length === 1 ? 'import.folderOne' : 'import.folderMany')
    : '';
  const importLabel = ok
    ? t('import.requests', { count: parsed.collection.count, requestLabel })
    : '';
      ...
      <div className="qa-modal qa-import" onMouseDown={(ev) => ev.stopPropagation()}>
        <div className="qa-modal-head">
          <span className="qa-modal-title">
            <Icon name="download" size={15} /> {t('import.title')}
          </span>
        ...
        <div className="qa-import-body">
          <div
            className={'qa-import-drop' + (drag ? ' is-drag' : '')}
            onDragOver={(e) => {
              e.preventDefault();
              setDrag(true);
            }}
          ...
          {parsed && parsed.error && (
            <div className="rn-data-err">
              <Icon name="x" size={12} /> {parsed.error}
            </div>
          )}
          {ok && (
            <div className="qa-import-preview">
              <div className="qa-import-preview-head">
                <span
                  className="qa-cred-type"
                  data-type={parsed.collection.source === 'postman' ? 'bearer' : 'basic'}
                >
                  {parsed.format}
                </span>
                ...
                <span className="qa-meta">
                  {t('import.preview', {
                    requests: parsed.collection.count,
                    requestLabel,
                    folders: parsed.collection.folders.length,
                    folderLabel,
                  })}
                </span>
              </div>
              {parsed.collection.baseUrl && (
                <div className="qa-env-url">{parsed.collection.baseUrl}</div>
              )}
            </div>
          )}
        </div>
