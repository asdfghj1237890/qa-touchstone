...
const VLABEL: Record<RateLimitVerdict, string> = {
  pass: 'rl.verdict.pass',
  vuln: 'rl.verdict.vuln',
  inconclusive: 'rl.verdict.inconclusive',
};

/** collection 內可加入測試的 request 摘要。 */
type ReqOption = { reqId: string; method: string; path: string };

function allRequests(): ReqOption[] {
  return (window.QA.COLLECTIONS || []).flatMap((c: any) =>
    (c.folders || []).flatMap((f: any) =>
      (f.requests || []).map((r: any) => ({ reqId: r.id, method: r.method, path: r.path }))
    )
  );
}
...
/** 面板每個 test 的結果 cell（RateLimitResult + burst 進度）。 */
export interface RateLimitCellState extends RateLimitResult {
  progress?: { done: number; n: number } | null;
}

export type RateLimitResultsMap = Record<string, RateLimitCellState>;

export interface RateLimitPanelProps {
  identities: Identity[];
  rateLimit: RateLimitConfig;
  setRateLimit: React.Dispatch<React.SetStateAction<RateLimitConfig>>;
  onFindings?: (findings: UnionFinding[]) => void;
  results?: RateLimitResultsMap | null;
  setResults?: React.Dispatch<React.SetStateAction<RateLimitResultsMap>>;
  onRunTest?: (
    test: RateLimitTest,
    ctx: { runner: () => Promise<any>; signal: AbortSignal }
  ) => void | Promise<void>;
  env?: QaEnv;
  vars?: any;
  cookies?: QaCookie[];
...
function RateLimitPanel({
  ...
  const { t } = useI18n();
  const tests = useMemo(() => rateLimit.tests || [], [rateLimit.tests]);
  const [localResults, setLocalResults] = useS<RateLimitResultsMap>({});
  ...
  const setResults = (controlled ? setResultsProp : setLocalResults) as React.Dispatch<
    React.SetStateAction<RateLimitResultsMap>
  >;
  const [running, setRunning] = useS<string | null>(null); // testId currently bursting
  const [confirming, setConfirming] = useS<RateLimitTest | null>(null); // test pending confirmation
  const abortRef = useRef<AbortController | null>(null);

  const setTests = (updater: RateLimitTest[] | ((ts: RateLimitTest[]) => RateLimitTest[])) =>
    setRateLimit((s) => ({
      ...s,
      tests: typeof updater === 'function' ? updater(s.tests || []) : updater,
    }));
  const addTest = (r: ReqOption) =>
    setTests((ts) =>
      ts.some((x) => x.reqId === r.reqId)
        ? ts
        : [
            ...ts,
            {
              id: `rl_${Date.now()}_${rlSeq++}`,
              reqId: r.reqId,
              method: r.method,
              path: r.path,
  ...
  const patchTest = (id: string, patch: Partial<RateLimitTest>) =>
    setTests((ts) => ts.map((x) => (x.id === id ? { ...x, ...patch } : x)));

  const idName = (id: string | undefined) => {
    const i = identities.find((x) => x.id === id);
    return i ? i.name || i.id : id;
  };

  const doRun = async (test: RateLimitTest) => {
    ...
    if (onRunTest) {
      try {
        await onRunTest(test, { runner, signal: controller.signal });
      } finally {
        setRunning(null);
      }
      return;
    ...
    try {
      const { responses, stats } = await runBurst(test, runner, {
        ...
        onProgress: (done, n) =>
          setResults((r) => ({
            ...r,
            [test.id]: { ...(r[test.id] || {}), progress: { done, n } },
          })),
      });
  ...
  return (
    <div className="qa-rl">
      <div className="qa-sec-head">
        <div>
          <h2>{t('security.mode.ratelimit')}</h2>
          <p>{t('rl.subtitle')}</p>
        </div>
      </div>

      <div className="qa-sec-summary">
        {(['total', 'vuln', 'pass', 'inconclusive'] as const).map((k) => (
          <span key={k} className={`qa-sec-chip qa-sec-chip--${k}`}>
            {summary[k] || 0} {k === 'total' ? t('rl.summary.total') : t('rl.verdict.' + k)}
          </span>
        ))}
      </div>
          ...
          <div key={test.id} className="qa-rl-test">
            ...
            <div className="qa-rl-cfg">
              <label>
                ...
                <select
                  ...
                  onChange={(e) => patchTest(test.id, { identityId: e.target.value })}
                >
                  {identities.map((i) => (
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    patchTest(test.id, {
                      n: Math.max(1, Math.min(MAX_N, parseInt(e.target.value, 10) || 1)),
                    })
                  }
                />
              </label>
                  ...
                  onChange={(e) =>
                    patchTest(test.id, {
                      concurrency: Math.max(
                        1,
                        Math.min(MAX_CONCURRENCY, parseInt(e.target.value, 10) || 1)
                      ),
                    })
                  }
                />
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) => patchTest(test.id, { sensitivity: e.target.value })}
                >
              ...
              {isRunning ? (
                <button className="qa-btn qa-btn--danger qa-btn--sm" onClick={stop}>
                  <Icon name="stop" size={12} /> {t('security.stop')}
                </button>
              ) : (
                <button
                  className="qa-btn qa-btn--primary qa-btn--sm"
                  onClick={() => setConfirming(test)}
                >
                  <Icon name="play" size={12} /> {t('rl.run')}
                </button>
              )}
            </div>

            {res && res.progress && isRunning && (
              <div className="qa-rl-progress">
                <div
                  className="qa-rl-progress-bar"
                  style={{ width: `${Math.round((res.progress.done / res.progress.n) * 100)}%` }}
                />
              </div>
            )}
            ...
            {res && res.stats && (
              <div className="qa-rl-result">
                <span className={`qa-rl-verdict qa-rl-verdict--${res.verdict}`}>
                  {t(VLABEL[res.verdict as RateLimitVerdict] || 'rl.verdict.inconclusive')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.sent} {t('rl.stat.sent')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.ok2xx} {t('rl.stat.ok')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.c429} {t('rl.stat.c429')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.headerHit ? '✓' : '—'} {t('rl.stat.throttleHdr')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.net} {t('rl.stat.err')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.avgMs} {t('rl.stat.avg')}
                </span>
                <span className="qa-rl-stat">
                  {res.stats.maxMs} {t('rl.stat.max')}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="qa-sec-modal" onClick={() => setConfirming(null)}>
          <div className="qa-sec-modal-body qa-rl-confirm" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              {t('rl.confirmBody', {
                n: confirming.n,
                concurrency: confirming.concurrency,
                target: `${confirming.method} ${confirming.path}`,
                identity: idName(confirming.identityId),
              })}
            </p>
            <div className="qa-rl-confirm-actions">
              <button className="qa-link" onClick={() => setConfirming(null)}>
                {t('rl.cancel')}
              </button>
              <button
