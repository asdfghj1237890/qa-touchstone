...
import type {
  Finding, Identity, RateLimitConfig, RateLimitResult, RateLimitTest, RateLimitVerdict, UnionFinding,
} from './types';
import type { QaCookie, QaEnv } from './state/WorkspaceContext';

const { useState: useS, useEffect: useE, useMemo, useRef } = React;
const VLABEL: Record<RateLimitVerdict, string> = { pass: 'rl.verdict.pass', vuln: 'rl.verdict.vuln', inconclusive: 'rl.verdict.inconclusive' };
...
function allRequests(): ReqOption[] {
  return (window.QA.COLLECTIONS || []).flatMap((c: any) =>
    (c.folders || []).flatMap((f: any) => (f.requests || []).map((r: any) => ({ reqId: r.id, method: r.method, path: r.path }))));
}
...
export interface RateLimitPanelProps {
  ...
  onRunTest?: (test: RateLimitTest, ctx: { runner: () => Promise<any>; signal: AbortSignal }) => void | Promise<void>;
  env?: QaEnv;
  vars?: any;
  cookies?: QaCookie[];
  sslVerify?: boolean;
...
function RateLimitPanel({ identities, rateLimit, setRateLimit, onFindings, results: resultsProp, setResults: setResultsProp, onRunTest,
                          env = { label: 'None', baseUrl: '' }, vars, cookies = [], sslVerify = true }: RateLimitPanelProps) {
  const { t } = useI18n();
  ...
  const setResults = (controlled ? setResultsProp : setLocalResults) as React.Dispatch<React.SetStateAction<RateLimitResultsMap>>;
  ...
  const setTests = (updater: RateLimitTest[] | ((ts: RateLimitTest[]) => RateLimitTest[])) =>
    setRateLimit(s => ({ ...s, tests: typeof updater === 'function' ? updater(s.tests || []) : updater }));
  const addTest = (r: ReqOption) => setTests(ts => ts.some(x => x.reqId === r.reqId)
    ? ts
  ...
  const patchTest = (id: string, patch: Partial<RateLimitTest>) => setTests(ts => ts.map(x => x.id === id ? { ...x, ...patch } : x));

  const idName = (id: string | undefined) => { const i = identities.find(x => x.id === id); return i ? (i.name || i.id) : id; };

  const doRun = async (test: RateLimitTest) => {
    const identity = identities.find(x => x.id === test.identityId) || identities[0];
    const runner = () => qaRunSavedRequest({ id: test.reqId as string }, {
      env, vars, cookies: cookies as QaRunCookie[], sslVerify, authOverride: (identity && identity.auth) as any, oauthToken: (identity && identity._oauthToken) as any,
    });
    const controller = new AbortController();
    abortRef.current = controller;
    setRunning(test.id);
    if (onRunTest) { try { await onRunTest(test, { runner, signal: controller.signal }); } finally { setRunning(null); } return; }
    setResults(r => ({ ...r, [test.id]: { progress: { done: 0, n: test.n as number }, stats: null, verdict: null } }));
    try {
  ...
  const allFindings = useMemo(() => tests.map(t0 => results[t0.id] && results[t0.id].finding).filter(Boolean) as Finding[], [results, tests]);
          ...
          <div key={test.id} className="qa-rl-test">
            ...
            {res && res.stats && (
              <div className="qa-rl-result">
                <span className={`qa-rl-verdict qa-rl-verdict--${res.verdict}`}>{t(VLABEL[res.verdict as RateLimitVerdict] || 'rl.verdict.inconclusive')}</span>
