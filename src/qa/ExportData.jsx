...
// ── QA Companion — export a collection to Postman v2.1 collection JSON ─────
...
function buildPostmanCollection(col, env) {
  const { REQUEST_DETAILS } = window.QA;
  const baseUrl = (col.baseUrl) || (env && env.baseUrl) || '';
  const toItem = (r) => {
    const det = REQUEST_DETAILS[r.id] || {};
    const headers = (det.headers || []).map(h => ({ key: h.key, value: h.value || '', disabled: h.on === false }));
    const item = {
      name: r.name,
      request: {
        method: r.method,
  ...
  return {
    info: {
      ...
      _exporter_id: 'QA Companion',
    },
    item: col.folders.map(f => ({ name: f.name, item: f.requests.map(toItem) })),
  };
}

function exportCollection(col, env) {
