...
function Dropdown({ value, options, onChange, className, renderOption, renderValue }: DropdownProps) {
  ...
  React.useEffect(() => {
    ...
    const target = menuRef.current.querySelector<HTMLElement>('[data-active="1"]') || menuRef.current.querySelector<HTMLElement>('button');
  ...
  const onMenuKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    else if (e.key === 'ArrowDown') { e.preventDefault(); if (items[idx + 1] || items[0]) (items[idx + 1] || items[0]).focus(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); if (items[idx - 1] || items[items.length - 1]) (items[idx - 1] || items[items.length - 1]).focus(); }
...
const LLM_DEFAULT_CFG: LlmCfg = { provider: 'builtin', model: 'claude-haiku-4-5', key: '', baseUrl: 'https://api.openai.com/v1/chat/completions' };
const LLM_PROVIDERS = [
...
function saveLlmCfg(cfg: LlmCfg): void { saveStorageJSON('qa_llm_cfg', cfg); }
