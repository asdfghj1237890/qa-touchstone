...
const ICON_PATHS: Record<string, string> = {
  ...
  eyeOff:
    'M3 3l18 18M10.6 10.7a3 3 0 0 0 4.2 4.2M9.4 5.2A9.6 9.6 0 0 1 12 5c6.5 0 10 7 10 7a18 18 0 0 1-3.1 3.9M6.1 6.2A18 18 0 0 0 2 12s3.5 7 10 7a9.3 9.3 0 0 0 2.6-.4',
  ...
  sparkle:
    'M12 3l1.9 4.6L18.5 9.5l-4.6 1.9L12 16l-1.9-4.6L5.5 9.5l4.6-1.9L12 3Z|M19 14l.8 2.2 2.2.8-2.2.8L19 20l-.8-2.2-2.2-.8 2.2-.8L19 14Z',
  circleHelp:
    'M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Z|M9.1 9a3 3 0 0 1 5.8 1c0 2-3 2.1-3 4M12 17h.01',
  users:
    'M16 19a4 4 0 0 0-8 0M12 11a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7ZM22 19a3.5 3.5 0 0 0-4.5-3.3M18 10.8A3.5 3.5 0 0 0 18 4.2',
...
export interface IconProps {
  ...
  className?: string;
  /** Dropdown 會在 chevron 圖示上多傳 data-open（Icon 本身不使用，僅供型別放行）。 */
  'data-open'?: string;
}

function Icon({ name, size = 16, stroke = 2, style, className }: IconProps) {
...
export interface PulseLogoProps {
  busy?: boolean;
  /** 成功送出當下的 timestamp；變動時觸發 1 秒綠色閃爍。 */
  flashAt?: number;
  active?: boolean;
  size?: number;
}

function PulseLogo({ busy, flashAt, active, size = 22 }: PulseLogoProps) {
  const [flash, setFlash] = React.useState(false);
  React.useEffect(() => {
    if (!flashAt) return;
    const start = setTimeout(() => setFlash(true), 0);
    const end = setTimeout(() => setFlash(false), 1000);
    return () => {
      clearTimeout(start);
      clearTimeout(end);
    };
    ...
    >
      <defs>
        ...
        <linearGradient id="qa-logo-stroke" x1="0" y1="0" x2="1" y2="0.4">
          ...
          <stop offset="0.55" stopColor="#4d9fff" />
      ...
      <g transform="translate(214,268) scale(21)" strokeLinecap="round" strokeLinejoin="round">
        ...
        />
        <path
          className="qa-pulse-sweep"
          d={PULSE_PATH}
          pathLength="100"
          stroke="url(#qa-logo-stroke)"
          strokeWidth="2.2"
          filter="url(#qa-logo-glow)"
        />
      </g>
    </svg>
  );
...
function StatusPill({ code, text, big }: StatusPillProps) {
...
export interface MiniCheckProps {
  on?: boolean;
  onClick?: () => void;
  checked?: boolean;
...
function MiniCheck({ on, onClick, checked, onChange }: MiniCheckProps) {
  const { t } = useI18n();
  ...
  return (
    <button
      onClick={handleClick}
      className="qa-minicheck"
      data-on={active ? '1' : '0'}
      aria-label={t('common.toggleRow')}
    >
...
export interface DropdownOption {
  ...
  title?: string;
  /** 呼叫端（如 PerfTest 的 renderOption/renderValue）可附帶額外欄位。 */
  [key: string]: unknown;
}
...
function Dropdown({
  ...
  React.useEffect(() => {
    ...
    const target =
      menuRef.current.querySelector<HTMLElement>('[data-active="1"]') ||
  ...
  const closeAndRefocus = () => {
    setOpen(false);
    const btn = ref.current && ref.current.querySelector<HTMLElement>('.qa-dd-btn');
    if (btn) btn.focus();
  };
...
function fmtBytesShared(b: number): string {
  b = Math.round(b);
  if (b <= 0) return '0 B';
...
function loadLlmCfg(): LlmCfg {
  const raw = loadStorageJSON<Partial<LlmCfg>>('qa_llm_cfg', {});
  return { ...LLM_DEFAULT_CFG, ...(raw && typeof raw === 'object' ? raw : {}) };
}
function saveLlmCfg(cfg: LlmCfg): void {
  saveStorageJSON('qa_llm_cfg', cfg);
}
