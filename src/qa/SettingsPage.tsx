  ...
  return (
    ...
    <div className="qa-set-grid" style={{ marginTop: 14 }}>
      <section className="qa-panel">
        ...
        <div className="qa-set-body">
          <KVTable rows={vars.globals} onChange={(g: any) => setVars({ ...vars, globals: g })} keyPh={t('settings.env.variable')} valPh={t('settings.env.value')} />
      ...
      <section className="qa-panel">
        <div className="qa-panel-head"><span><Icon name="layers" size={14} /> {t('settings.env.collectionVars')}</span>
          <div style={{ width: 168 }}><Dropdown value={colEdit} options={colList.map((c: any) => ({ value: c.id, label: c.name }))} onChange={setColEdit} /></div>
        </div>
        <div className="qa-set-body">
          <KVTable rows={colVars} onChange={(list: any) => setVars({ ...vars, collections: { ...vars.collections, [colEdit]: list } })} keyPh={t('settings.env.variable')} valPh={t('settings.env.value')} />
        </div>
      </section>
    </div>
    ...
    <div className="qa-set-grid" style={{ marginTop: 18 }}>
      <section className="qa-panel">
        ...
        <div className="qa-set-body">
          <KVTable rows={vars.environments[envEdit] || []} onChange={(list: any) => setVars({ ...vars, environments: { ...vars.environments, [envEdit]: list } })} keyPh={t('settings.env.variable')} valPh={t('settings.env.value')} />
        </div>
      </section>
      <section className="qa-panel">
...
function CookieSettings({ cookies, setCookies }: CookieSettingsProps) {
  const { t } = useI18n();
  const blank = (): JarCookie => ({ id: 'ck' + Date.now().toString(36), name: '', value: '', domain: '', path: '/', expires: '', httpOnly: false, secure: true, sameSite: 'Lax', on: true });
  const upd = (id: string | undefined, d: Partial<JarCookie>) => setCookies(cookies.map(c => c.id === id ? { ...c, ...d } : c));
...
function ApiSettings() {
  ...
  const typeLabel: Record<string, string> = { aws: t('settings.api.type.aws'), bearer: t('settings.api.type.bearer'), basic: t('settings.api.type.basic') };
          ...
          {p.type === 'aws' && (
            <>
              <FieldRow label={t('settings.api.awsProfile')}>
                <Dropdown value={p.name} options={profiles.filter((x: any) => x.type === 'aws').map((x: any) => x.name)} onChange={() => {}} />
              </FieldRow>
          ...
          <FieldRow label={t('settings.appearance.accent')}>
            <div className="tw-accents">
              ...
              {Object.entries(ACC).map(([name, hex]) => (
                <button key={name} className="tw-acc" data-active={accent === hex ? '1' : '0'} style={{ background: hex as string }} onClick={() => setAccent(hex as string)} title={name} />
              ))}
            </div>
          </FieldRow>
          ...
          {!builtin && (
            <>
              <FieldRow label={t('settings.llm.model')}>
                {cfg.provider === 'openai'
                  ? <Dropdown value={cfg.model || 'gpt-5.4-mini'} options={MODELS.openai} onChange={(m: string) => save({ model: m })} />
                  : <input className="qa-inp" value={cfg.model} onChange={e => save({ model: e.target.value })} placeholder="model id, e.g. llama-3.1-70b" />}
              </FieldRow>
              <FieldRow label={t('settings.llm.apiKey')}><SecretInput value={cfg.key} onChange={(v: string) => save({ key: v })} placeholder="sk-…" /></FieldRow>
              {cfg.provider === 'custom' && (
...
export function PrivacySettings() {
  ...
  const save = (d: Partial<PrivacyCfg>) => { const n = { ...cfg, ...d }; setCfg(n); savePrivacyCfg(n); };
          ...
          {policy.effectiveMode === 'local' && dest.provider === 'custom' && dest.isCloud && (
            <>
              <FieldRow label={t('settings.privacy.attest')}>
                <MiniCheck checked={cfg.selfManagedAttested} onChange={(v: boolean) => save({ selfManagedAttested: v })} />
              </FieldRow>
...
function SettingsPage({ accent, setAccent, initialTab, vars, setVars, cookies, setCookies, sslVerify, setSslVerify }: SettingsPageProps) {
  const { t } = useI18n();
