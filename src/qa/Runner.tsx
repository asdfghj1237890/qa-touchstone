...
function Runner({ env, vars, tests, cookies = [], sslVerify = true, oauthTokens = {} }: RunnerProps) {
  const { t } = useI18n();
  ...
  useEffRN(() => { setSelIds((COLLECTIONS.find((c: any) => c.id === colId) || COLLECTIONS[0]).folders.flatMap((f: any) => f.requests).map((r: any) => r.id)); }, [colId]);
  ...
  const toggleReq = (id: string) => setSelIds(s => s.includes(id) ? s.filter(x => x !== id) : [...s, id]);

  let dataRows: any[] | null = null, dataErr = '', dataCols: string[] = [], dataFmt: 'json' | 'csv' | null = null;
  if (dataText.trim()) {
  ...
  const run = () => {
    ...
    for (let i = 0; i < iters; i++) reqs.forEach((r: any) => queue.push({ iter: i + 1, r, data: dataRows ? dataRows[i] : null }));
    ...
    (async () => {
      ...
      for (let k = 0; k < queue.length; k++) {
        ...
        let resp;
        try {
          resp = await qaRunSavedRequest(r, { env, vars, cookies: cookies as QaRunCookie[], sslVerify, oauthToken: oauthTokens[r.id], collectionId: colId, localVars: data || {} });
        } catch (e) {
        ...
        acc.push({
          ...
          passed: res.filter((x: any) => x.pass).length, total: res.length,
        });
  ...
  return (
    <div className="rn">
      <div className="rn-config">
        ...
        <Dropdown value={colId} options={COLLECTIONS.map((c: any) => ({ value: c.id, label: c.name }))} onChange={setColId} />
