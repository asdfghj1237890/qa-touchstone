...
function Runner({
  ...
  const [colId, setColId] = useRN<string>(COLLECTIONS[0].id);
  const col = COLLECTIONS.find((c: any) => c.id === colId) || COLLECTIONS[0];
  const colReqs = col.folders.flatMap((f: any) => f.requests);
  const [selIds, setSelIds] = useRN<string[]>(colReqs.map((r: any) => r.id));
  ...
  const fileRef = useRefRN<HTMLInputElement | null>(null);
  const [phase, setPhase] = useRN<'idle' | 'running' | 'done'>('idle');
  const [progress, setProgress] = useRN(0);
  const [results, setResults] = useRN<RunnerRow[]>([]);
  const timerRef = useRefRN<{ cancel?: () => void } | null>(null);
  ...
  useEffRN(() => {
    setSelIds(
      (COLLECTIONS.find((c: any) => c.id === colId) || COLLECTIONS[0]).folders
  ...
  useEffRN(
    () => () => {
      if (timerRef.current && timerRef.current.cancel) timerRef.current.cancel();
    },
    []
  );
  ...
  const run = () => {
    ...
    const reqs = colReqs.filter((r: any) => selIds.includes(r.id));
    if (!reqs.length) return;
    startedRef.current = true;
    const queue: Array<{ iter: number; r: any; data: any }> = [];
    for (let i = 0; i < iters; i++)
      reqs.forEach((r: any) => queue.push({ iter: i + 1, r, data: dataRows ? dataRows[i] : null }));
    setResults([]);
    setProgress(0);
    setPhase('running');
    const acc: RunnerRow[] = [];
      ...
      try {
        for (let k = 0; k < queue.length; k++) {
          ...
          acc.push({
            ...
            status: resp.status,
            time: resp.time,
            passed: res.filter((x: any) => x.pass).length,
            total: res.length,
          });
          setResults([...acc]);
          setProgress((k + 1) / queue.length);
  ...
  const failed = results.filter(
    (r) => (r.total > 0 && r.passed < r.total) || r.status >= 400
  ).length;
    ...
    <div className="rn">
      <div className="rn-config">
        ...
        <div className="rn-sec">
          <span>{t('runner.requests')}</span>
          <span className="qa-meta">
            {selIds.length}/{colReqs.length}
          </span>
        </div>
        <div className="rn-reqs">
          {colReqs.map((r: any) => (
            <div className="rn-req" key={r.id}>
              ...
              {(tests[r.id] || []).length > 0 && (
                <span className="rn-req-tests" title={t('runner.assertions')}>
                  {(tests[r.id] || []).length}✓
                </span>
              )}
            </div>
        ...
        <div className="rn-row2">
          ...
          <div className="pf-param">
            ...
            <input
              ...
              value={delay}
              disabled={running}
              onChange={(e) => setDelay(Math.max(0, +e.target.value || 0))}
            />
          </div>
        </div>

        <div className="rn-sec">
          <span>{t('runner.dataIteration')}</span>
          <span className="qa-meta">
            {dataRows
              ? t('runner.rowsCols', { rows: dataRows.length, cols: dataCols.length })
              : t('runner.optional')}
          </span>
        </div>
        <div className="rn-data-tools">
          ...
          {dataName && (
            <span className="rn-data-file">
              <Icon name="fileText" size={12} /> {dataName}{' '}
              <button onClick={clearData} aria-label={t('runner.clearData')}>
                <Icon name="x" size={11} />
              </button>
            </span>
          )}
        ...
        {dataErr && (
          <div className="rn-data-err">
            <Icon name="x" size={12} /> {dataErr}
          </div>
        )}
        {dataRows && (
          <div className="rn-data-preview">
            <div className="rn-data-ok">
              <Icon name="check" size={12} />{' '}
              {t('runner.dataOk', { format: dataFmt!.toUpperCase(), rows: dataRows.length })}
            </div>
            <div
              className="rn-data-grid"
              style={{ gridTemplateColumns: `28px repeat(${dataCols.length}, minmax(60px, 1fr))` }}
            >
              <div className="rn-data-cell rn-data-hcell">#</div>
              {dataCols.map((c) => (
                <div key={c} className="rn-data-cell rn-data-hcell">
                  <code>{`{{${c}}}`}</code>
                </div>
              ))}
              {dataRows.slice(0, 5).map((row, i) => (
                <React.Fragment key={i}>
                  ...
                  {dataCols.map((c) => (
                    <div key={c} className="rn-data-cell">
            ...
            {dataRows.length > 5 && (
              <div className="rn-data-more">
                {t('runner.moreRows', { count: dataRows.length - 5 })}
              </div>
            )}
          </div>
        )}
        ...
        {phase !== 'idle' && (
          <div className="pf-prog">
            ...
            <div className="pf-prog-meta">
              ...
              <span>{running ? t('perf.running') : t('runner.done')}</span>
            </div>
          </div>
        )}
      </div>
        ...
        {phase === 'idle' ? (
          <div className="pf-empty">
            <div className="pf-empty-icon">
              <Icon name="play" size={26} stroke={1.5} />
            </div>
            <div className="pf-empty-title">{t('runner.emptyTitle')}</div>
            <div className="pf-empty-sub">
              {t('runner.emptySub', {
                collection: col.name,
                requests: selIds.length,
                requestLabel: emptyRequestLabel,
                iterations: iters,
                iterationLabel,
              })}
            </div>
          ...
          <>
            <div className="rn-summary">
              <div className="rn-sum">
                <span>{t('runner.requests')}</span>
                <strong>{total}</strong>
              </div>
              <div className="rn-sum">
                <span>{t('runner.assertions')}</span>
                <strong
                  style={{
                    color: aPass === aTotal ? 'oklch(0.78 0.14 150)' : 'oklch(0.74 0.16 22)',
                  }}
                >
                  {aPass}/{aTotal}
                </strong>
              ...
              <div className="rn-sum">
                <span>{t('runner.failures')}</span>
                <strong style={{ color: failed ? 'oklch(0.74 0.16 22)' : 'var(--text)' }}>
                  {failed}
                </strong>
              </div>
              <div className="rn-sum">
                <span>{t('runner.avgTime')}</span>
                <strong>
                  {avg}
                  <em> ms</em>
                </strong>
            ...
            <div className="rn-table">
              {results.map((r, i) => {
                const head = i === 0 || r.iter !== results[i - 1]?.iter;
                return (
                  <React.Fragment key={i}>
                    {head && (
                      <div className="rn-iter-head">
                        {t('runner.iteration', { number: r.iter })}
                        {dataRows ? ` · ${JSON.stringify(dataRows[r.iter - 1])}` : ''}
                      </div>
                    )}
                    <div className="rn-trow qa-fade">
                      ...
                      <span
                        className="rn-tstatus"
                        style={{ color: window.QATheme.statusColor(r.status) }}
                      >
                        {r.status}
                      </span>
                      <span className="rn-ttime">{r.time}ms</span>
                      {r.total > 0 ? (
                        <span className="rn-ttests" data-pass={r.passed === r.total ? '1' : '0'}>
                          <Icon name={r.passed === r.total ? 'check' : 'x'} size={11} stroke={3} />{' '}
                          {r.passed}/{r.total}
                        </span>
                      ) : (
