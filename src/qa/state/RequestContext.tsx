...
export function RequestProvider({ children }: { children: React.ReactNode }) {
  ...
  const [history, setHistory] = React.useState<HistoryEntry[]>(() =>
    window.QA.SEED_HISTORY.map((h: HistoryEntry) => ({ ...h }))
  );
  ...
  localList.forEach((v) => {
    if (v.on !== false && v.key) localObj[v.key] = v.value;
  });
  const activeMap: Record<string, string> = window.qaVarMap!(
    vars,
    env.label,
    collectionId,
    localObj
  ...
  const reqUrlForHost = window.qaSubstitute
    ? window.qaSubstitute(req.url || '', activeMap)
    : req.url || '';
  const reqIsAbsolute = /^https?:\/\//i.test(reqUrlForHost);
  const envBaseSub = window.qaSubstitute
    ? window.qaSubstitute(String(env.baseUrl || ''), activeMap)
    : String(env.baseUrl || '');
  ...
  const addTestsForCase = (method: string, path: string, assertions: QaAssertion[]): boolean => {
    const all = window.QA.COLLECTIONS.flatMap((c: any) =>
  ...
  const showCookieToast = (detail: { name: string; domain: string }) => {
    try {
      window.dispatchEvent(new CustomEvent(COOKIE_TOAST_EVENT, { detail }));
    } catch {
      /* 非瀏覽器環境 */
    }
  };
  ...
  const send = () => {
    ...
    setResponse(null);
    executeRequest(sentReq, env, activeMap, {
      cookies: reqCookies,
      sslVerify,
      oauthToken: oauthTokens[sentReq.id],
    }).then((resp: any) => {
      ...
      const pathWithParams =
        sentReq.url +
        (sentReq.params.filter((p) => p.on && p.key).length
          ? '?' +
            sentReq.params
              .filter((p) => p.on && p.key)
              .map((p) => `${p.key}=${p.value}`)
      ...
      if (perHop && perHop.length) {
        ...
        if (!fullSentUrl) {
          const sentUrlSub = window.qaSubstitute
            ? window.qaSubstitute(sentReq.url || '', activeMap)
            : sentReq.url || '';
          const sentIsAbsolute = /^https?:\/\//i.test(sentUrlSub);
          const envBaseSubCap = window.qaSubstitute
            ? window.qaSubstitute(String(env.baseUrl || ''), activeMap)
            : String(env.baseUrl || '');
        ...
        let sentHost = '',
          sentPath = '/';
        try {
          const u = new URL(fullSentUrl);
          sentHost = u.hostname;
          sentPath = u.pathname || '/';
        } catch {
          sentHost = hostOf(fullSentUrl);
        }
        const scList = Array.isArray(sc)
          ? sc
          : String(sc)
              .split('\n')
              .map((s: string) => s.trim())
  ...
  const fetchOAuthToken = async (targetReq: QaRequest): Promise<OAuthToken> => {
    const token = await requestOAuthToken(targetReq.auth.oauth2, activeMap, {
      sslVerify,
      executeRequest,
    });
