...
// ── QA Companion — auto-generated API documentation from a collection ─────
...
function docAuthLabel(authType, t) {
  return {
...
function buildDocsHtml(col, env, t = (key) => key) {
  ...
  const sections = reqs.map(r => {
    ...
    const resp = window.QA.RESPONSES[r.id];
    const params = (det.params || []).filter(p => p.key);
    const pTable = params.length ? `<table><tr><th>${esc(t('docs.name'))}</th><th>${esc(t('docs.example'))}</th><th>${esc(t('docs.required'))}</th></tr>${params.map(p => `<tr><td><code>${esc(p.key)}</code></td><td>${esc(p.value || '—')}</td><td>${p.on === false ? esc(t('common.optional')) : esc(t('common.yes'))}</td></tr>`).join('')}</table>` : '';
