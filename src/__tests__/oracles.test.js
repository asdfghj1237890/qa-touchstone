...
describe('worstSeverity', () => {
  it('returns the highest-ranked severity in a finding list, or null when empty', () => {
    expect(worstSeverity([{ severity: 'low' }, { severity: 'high' }, { severity: 'medium' }])).toBe(
      'high'
    );
...
describe('scanSensitive — secrets', () => {
  ...
  it('flags secret-named fields by key, even when the value is an object', () => {
    const f = scanSensitive(
      resp({ password: 'hunter2', nested: { client_secret: { rotated: true } } })
    );
...
describe('scanSensitive — PII', () => {
  ...
  it('flags only Luhn-valid card numbers', () => {
    expect(
      scanSensitive(resp({ c: '4242424242424242' })).some(
        (x) => x.title === 'Credit-card-like number'
      )
    ).toBe(true);
    expect(
      scanSensitive(resp({ c: '4242424242424241' })).some(
        (x) => x.title === 'Credit-card-like number'
      )
    ).toBe(false);
  });
  it('still flags a Luhn-valid non-card number (documented over-match; over-flagging is the safe direction)', () => {
    ...
    expect(
      scanSensitive(resp({ orderId: '4242424242424242' })).some(
        (x) => x.title === 'Credit-card-like number'
      )
    ).toBe(true);
  });
...
describe('scanSensitive — internal/debug', () => {
  ...
  it('flags leaky response headers', () => {
    const f = scanSensitive(
      resp({ ok: true }, { Server: 'nginx/1.2.3', 'X-Powered-By': 'Express' })
    );
...
describe('scanSensitive — config', () => {
  it('returns nothing when the sensitive oracle is disabled', () => {
    expect(
      scanSensitive(resp({ email: 'a@b.co' }), { ...DEFAULT_ORACLE_CONFIG, sensitive: false })
    ).toEqual([]);
  });
  it('applies a per-group severity override', () => {
    const f = scanSensitive(resp({ email: 'a@b.co' }), {
...
describe('checkSchema', () => {
  ...
  it('flags a missing baseline field as medium', () => {
    ...
    expect(f).toContainEqual(
      expect.objectContaining({ title: 'Missing field', path: 'name', severity: 'medium' })
    );
  });
  it('flags a type mismatch as medium', () => {
    const f = checkSchema({ id: '1', name: 's' }, base);
    expect(f).toContainEqual(
      expect.objectContaining({ title: 'Type mismatch', path: 'id', severity: 'medium' })
    );
  });
  ...
  it('returns nothing when the schema oracle is disabled', () => {
    expect(
      checkSchema({ id: 1, name: 's', extra: 1 }, base, { ...DEFAULT_ORACLE_CONFIG, schema: false })
    ).toEqual([]);
  });
});
...
describe('runOracles', () => {
  ...
  it('runs sensitive on any status but schema only on 2xx with a baseline', () => {
    const cell200 = {
      status: 200,
      response: { status: 200, body: { id: 1, surprise: 'x' }, headers: {} },
    };
    ...
    const cell403 = {
      status: 403,
      response: { status: 403, body: { id: 1, surprise: 'x' }, headers: {} },
    };
  ...
  it('cross-bumps an undeclared field that also leaks to high', () => {
    const cell = {
      status: 200,
      response: { status: 200, body: { id: 1, email: 'a@b.co' }, headers: {} },
    };
...
describe('summarizeFindings', () => {
  it('tallies findings by severity across the grid', () => {
    ...
    expect(summarizeFindings(results)).toEqual({
      total: 3,
      bySeverity: { info: 0, low: 1, medium: 0, high: 2, critical: 0 },
    });
  });
});
...
describe('scanSensitiveLLM', () => {
  ...
  it('wraps an LLM error', async () => {
    await expect(
      scanSensitiveLLM({ body: {} }, async () => {
        throw new Error('no key');
      })
    ).rejects.toThrow(/LLM scan failed/);
  });
});
