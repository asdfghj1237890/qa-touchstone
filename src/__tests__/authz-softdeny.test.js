...
describe('detectSoftDeny', () => {
  it('flags an explicit auth-denial phrase in a 2xx message field', () => {
    ...
    expect(detectSoftDeny({ status: 200, body: { message: 'You are not authorized to view this' } })).toBe('denied');
  ...
  it('flags a denial phrase nested in an errors array', () => {
    expect(detectSoftDeny({ status: 200, body: { errors: [{ message: 'Permission denied' }] } })).toBe('denied');
  });
  ...
  it('does NOT false-positive on benign prose that merely contains a denial word', () => {
    ...
    expect(detectSoftDeny({ status: 200, body: { title: 'Forbidden City walking tour', id: 7 } })).toBe(null);
    expect(detectSoftDeny({ status: 200, body: { description: 'Access control basics for beginners' } })).toBe(null);
  });
});
...
describe('classifyResponseOutcome', () => {
  ...
  it('reclassifies a soft-403 (200 + denial body) as denied', () => {
    expect(classifyResponseOutcome({ status: 200, body: { error: 'Access denied' } })).toBe('denied');
  });
...
describe('runMatrix — soft-deny awareness', () => {
  ...
  it('fails an allow-cell that returns a 200 soft-403 (legit user is actually blocked)', async () => {
    const runner = () => Promise.resolve({ status: 200, time: 1, body: { message: 'You are not authorized' } });
    const results = await runMatrix(
      { ...state, expect: { r1: { admin: 'allow', anon: 'skip' } } }, runner, {});
