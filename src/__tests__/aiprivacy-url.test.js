...
describe('redactUrlForAI', () => {
  ...
  it('collapses secret-like path segments', () => {
    expect(redactUrlForAI('https://h/api/users/bob@acme.com/profile')).toBe('/api/users/{id}/profile');
    expect(redactUrlForAI('https://h/keys/AKIAIOSFODNN7EXAMPLE/rotate')).toBe('/keys/{id}/rotate');
    expect(redactUrlForAI('https://h/reset/eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c')).toContain('{id}');
  });
