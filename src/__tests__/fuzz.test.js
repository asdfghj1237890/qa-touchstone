...
describe('classifyFuzzResponse', () => {
  ...
  it('flags a 5xx as a server error (the fuzz input broke the endpoint)', () => {
    expect(classifyFuzzResponse(p('long'), { status: 500, body: '' })).toMatchObject({
      signal: 'server-error',
      severity: 'high',
    });
  });
  it('flags a leaked stack trace / SQL error regardless of status', () => {
    expect(
      classifyFuzzResponse(p('sqli'), {
        status: 200,
        body: 'You have an error in your SQL syntax near ...',
      }).signal
    ).toBe('error-leak');
    expect(
      classifyFuzzResponse(p('xss'), { status: 200, body: 'Traceback (most recent call last):' })
        .signal
    ).toBe('error-leak');
  });
  it('flags reflected dangerous input (XSS echoed back) as high', () => {
    const r = classifyFuzzResponse(p('xss'), {
      status: 200,
  ...
  it('does not flag a clean, well-handled response', () => {
    expect(
      classifyFuzzResponse(p('sqli'), { status: 400, body: { error: 'invalid id' } }).signal
    ).toBe('ok');
  });
...
describe('fuzzFinding', () => {
  ...
  it('produces a fuzz finding for a server error', () => {
    const f = fuzzFinding(
      meta,
      { seedName: 'id', payload: FUZZ_PAYLOADS.find((p) => p.id === 'long') },
      { status: 500, body: '' }
    );
  ...
  it('returns null when the response is clean', () => {
    expect(
      fuzzFinding(
        meta,
        { seedName: 'id', payload: FUZZ_PAYLOADS.find((p) => p.id === 'sqli') },
...
describe('runFuzz', () => {
  const plan = {
    method: 'GET',
    path: '/items',
    seeds: [{ name: 'id', location: { kind: 'query', key: 'id' }, value: '1' }],
  };
  it('runs every case through the injected runner and collects findings', async () => {
    ...
    const runner = (seed, payload) =>
      Promise.resolve(
        payload.id === 'long' ? { status: 500, body: '' } : { status: 200, body: { ok: true } }
      );
