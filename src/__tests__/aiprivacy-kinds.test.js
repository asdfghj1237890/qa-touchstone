...
describe('AI_KINDS response-review', () => {
  const payload = {
    method: 'GET',
    url: 'https://api.acme.com/users/42831?token=abc',
    status: 200,
    statusText: 'OK',
    time: 12,
  ...
  it('redacted: url host + body value gone, but prompt still has shape', () => {
    const prompt = AI_KINDS['response-review'].buildPrompt(
      AI_KINDS['response-review'].redact(payload, ctxR)
    );
  ...
  it('full: sends raw body + url', () => {
    const prompt = AI_KINDS['response-review'].buildPrompt(
      AI_KINDS['response-review'].redact(payload, ctxF)
    );
...
describe('AI_KINDS testgen / triage / sensitive-scan', () => {
  it('testgen openapi drops servers', () => {
    const p = AI_KINDS['testgen'].buildPrompt(
      AI_KINDS['testgen'].redact(
        {
          source: 'openapi',
          input: JSON.stringify({ servers: [{ url: 'https://x.acme.com' }], paths: {} }),
        },
        ctxR
      )
    );
  ...
  it('triage redacts evidence text', () => {
    const p = AI_KINDS['triage'].buildPrompt(
      AI_KINDS['triage'].redact(
        { input: [{ i: 0, title: 't', evidence: 'leak a@b.com', path: 'a.b' }] },
        ctxR
      )
    );
