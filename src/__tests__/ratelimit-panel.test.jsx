...
function ControlledRL(props) {
  ...
  const onRunTest = async (test, { runner }) => {
    setResults((r) => ({
      ...r,
      [test.id]: { progress: { done: 0, n: test.n }, stats: null, verdict: null },
    }));
    const { responses, stats } = await runBurst(test, runner, {
      onProgress: (done, n) =>
        setResults((r) => ({ ...r, [test.id]: { ...(r[test.id] || {}), progress: { done, n } } })),
    });
  ...
  return (
    <RateLimitPanel {...props} results={results} setResults={setResults} onRunTest={onRunTest} />
  );
}
...
const rl = (n = 5, sensitivity = 'sensitive') => ({
  tests: [
    {
      id: 'rl1',
      reqId: 'r1',
...
describe('RateLimitPanel — runs on the canned path', () => {
    ...
    window.QA.COLLECTIONS = [
      {
        ...
        folders: [
          {
            name: 'F',
            requests: [{ id: 'r1', method: 'POST', name: 'login', path: 'https://api.test/login' }],
          },
        ],
      },
    ];
  ...
  it('requires the confirm modal before any burst, then flags a vuln + finding when unthrottled', async () => {
    window.QA.RESPONSES = {
      r1: { status: 200, statusText: 'OK', time: 2, size: 2, body: { ok: true }, headers: {} },
    };
    renderPanel(rl());
    ...
    await waitFor(() => expect(document.querySelector('.qa-rl-verdict--vuln')).not.toBeNull(), {
      timeout: 4000,
    });
  ...
  it('reports pass when a rate-limit header is present', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Run burst/i }));
    fireEvent.click(screen.getByRole('button', { name: /Send burst/i }));
    await waitFor(() => expect(document.querySelector('.qa-rl-verdict--pass')).not.toBeNull(), {
      timeout: 4000,
    });
  });
});
...
describe('RateLimitPanel — controlled-mode delegation', () => {
  ...
  it('delegates to onRunTest and does NOT run the local fallback (no double-run)', async () => {
    ...
    const onRunTestSpy = vi.fn(async () => {
