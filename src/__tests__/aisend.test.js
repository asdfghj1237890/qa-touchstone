...
const req = {
  ...
  payload: {
    method: 'GET',
    url: 'https://api.acme.com/users/7?token=abc',
    status: 200,
    statusText: 'OK',
    time: 1,
    expected: [],
...
describe('qaAiSend', () => {
  ...
  it('cancel from approval prevents any transport call', async () => {
    const callLLM = vi.fn().mockResolvedValue('ok');
    await expect(qaAiSend(req, { callLLM, approve: () => Promise.resolve(false) })).rejects.toThrow(
      /cancel/i
    );
  ...
  it('local mode blocks a cloud provider before sending', async () => {
    ...
    const callLLM = vi.fn().mockResolvedValue('ok');
    await expect(
      qaAiSend(req, { callLLM, approve: () => Promise.resolve(true) })
    ).rejects.toThrow();
    expect(callLLM).not.toHaveBeenCalled();
  });
  ...
  it('loads missing backend policy before approval or transport and fails closed when denied', async () => {
    ...
    const api = {
      getAiPolicy: vi.fn().mockResolvedValue({ externalAllowed: false, locked: true }),
    };
