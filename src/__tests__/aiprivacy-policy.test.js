...
describe('assertEgressAllowed', () => {
  ...
  it('redacted allows cloud', () => {
    expect(() => assertEgressAllowed('redacted', openai, cfg('redacted'))).not.toThrow();
  });
  it('local blocks cloud', () => {
    expect(() => assertEgressAllowed('local', openai, cfg('local'))).toThrow(EgressBlockedError);
  });
  it('local allows loopback', () => {
    expect(() => assertEgressAllowed('local', local, cfg('local'))).not.toThrow();
  });
  it('local blocks public custom unless attested', () => {
    expect(() => assertEgressAllowed('local', publicCustom, cfg('local'))).toThrow();
    expect(() =>
      assertEgressAllowed('local', publicCustom, cfg('local', { selfManagedAttested: true }))
    ).not.toThrow();
  });
});
