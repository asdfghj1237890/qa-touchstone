    ...
    await runBola({ identities: props.identities, tests: props.bola.tests }, runner, {
      ...
      onCell: (testId, attackerId, ownerId, cell) =>
        setResults((r) => {
          ...
          if (attackerId == null)
            return { ...r, [testId]: { ...tr, reference: { ...tr.reference, [ownerId]: cell } } };
          return {
            ...r,
            [testId]: {
              ...tr,
              attacks: {
                ...tr.attacks,
                [attackerId]: { ...(tr.attacks[attackerId] || {}), [ownerId]: cell },
              },
            },
          };
        }),
...
describe('BolaPanel — runs on the canned path', () => {
  ...
  it('runs the configured test and surfaces a confirmed BOLA cell + finding', async () => {
    ...
    await waitFor(() => expect(document.querySelector('.qa-bola-cell--vuln')).not.toBeNull(), {
      timeout: 4000,
    });
    // A finding badge / panel shows the object-authz finding.
    await waitFor(() =>
      expect(document.querySelector('.qa-sec-findpanel, .qa-bola-findpanel')).not.toBeNull()
    );
      ...
      tests: [
        {
          ...
          method: 'GET',
          path: 'https://api.test/users/42',
          idLocation: { kind: 'body', path: 'order.id' },
          idValues: { alice: 'A1' },
        },
      ],
    };
...
describe('BolaPanel — setup automation', () => {
  ...
  beforeEach(() => {
    ...
    window.QA.COLLECTIONS = [
      {
        ...
        folders: [
          {
            name: 'F',
            requests: [{ id: 'r2', method: 'GET', name: 'order', path: '/orders/123' }],
          },
        ],
      },
    ];
    ...
    window.QA.RESPONSES = {
      r2: { status: 200, statusText: 'OK', time: 1, size: 9, body: { id: '123' }, headers: {} },
    };
  });
  ...
  it('test 1: adding a test via the select shows the detection suggestion', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Detected id in/i)).not.toBeNull(), {
      timeout: 3000,
    });
  });
      ...
      () =>
        expect(
          document.querySelector(
            '.qa-bola-cell--vuln, .qa-bola-cell--unconfirmed, .qa-bola-cell--pass, .qa-bola-cell--inconclusive'
          )
        ).not.toBeNull(),
      { timeout: 8000 }
    );
