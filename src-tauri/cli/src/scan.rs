...
use qa_touchstone_core::security::lifecycle::{
    diff_runs, fingerprint, gate_count, scope_hash_of, snapshot_of, AnnotationsFile, Records,
    Snapshot, FP_VERSION,
};
use qa_touchstone_core::security::report::{
    build_report, report_to_html, report_to_junit, report_to_sarif,
};
use serde::Serialize;
...
fn build_scope_descriptor(cfg: &Config, env: Option<&str>) -> String {
    ...
    let req_shape = |id: &str| -> Value {
        match cfg.requests.iter().find(|r| r.id == id) {
            Some(r) => {
                let mut hk: Vec<String> = r.headers.iter().map(|k| k.key.clone()).collect();
                hk.sort();
                let mut qk: Vec<String> = r.query.iter().map(|k| k.key.clone()).collect();
                qk.sort();
                let body_mode = r
                    .body
                    .as_ref()
                    .map(|b| body_mode_token(&b.mode))
                    .unwrap_or("none");
    ...
    if let Some(m) = sec.and_then(|s| s.matrix.as_ref()) {
        ...
        eps.sort();
        for e in &eps {
            req_ids.insert(e.clone());
        }
        for rid in m.expect.keys() {
            req_ids.insert(rid.clone());
        }
        let mut deny = m.deny_set.clone();
        deny.sort();
        let mut expect: Vec<Value> = m
            .expect
            .iter()
            .flat_map(|(rid, row)| {
                row.iter()
                    .map(move |(idid, exp)| json!([rid, idid, expectation_token(exp)]))
            })
            .collect();
        expect.sort_by_key(|a| a.to_string());
        root.insert(
            "matrix".into(),
            json!({ "endpoints": eps, "denySet": deny, "expect": expect }),
        );
    }
    ...
    if let Some(o) = sec.and_then(|s| s.oracles.as_ref()) {
        ...
        root.insert(
            "oracles".into(),
            json!({ "sensitive": o.sensitive, "schema": o.schema, "severityOverrides": ov }),
        );
    }
    if let Some(bf) = sec.and_then(|s| s.bfla.as_ref()) {
        ...
        let mut eps_sorted = bfla_eps.clone();
        eps_sorted.sort();
        let mut deny = bf.deny_set.clone();
        deny.sort();
        root.insert(
            "bfla".into(),
            serde_json::json!({ "endpoints": eps_sorted, "denySet": deny }),
        );
    }
    if let Some(fz) = sec.and_then(|s| s.fuzz.as_ref()) {
        ...
        for t in &fz.targets {
            ...
            let body_val: Option<serde_json::Value> = req.body.as_ref().and_then(|b| {
                serde_json::from_str(&qa_substitute(&b.content, &var_map, &mut RealDynamics)).ok()
            });
        ...
        fuzz_entries.sort_by(|a, b| {
            a["request"]
                .as_str()
                .unwrap_or("")
                .cmp(b["request"].as_str().unwrap_or(""))
        });
        root.insert("fuzz".into(), serde_json::Value::Array(fuzz_entries));
    }
    let reqs: Map<String, Value> = req_ids
        .into_iter()
        .map(|id| {
            let sh = req_shape(&id);
            (id, sh)
...
) -> Vec<qa_touchstone_core::security::report::EngineReport> {
    engines
        .iter()
        .map(|e| qa_touchstone_core::security::report::EngineReport {
...
pub async fn run_scan(
    ...
    if cfg.security.is_none() {
        eprintln!("error: config has no `security` block");
        return ExitCode::from(2);
    }
    ...
    if let Some(eng) = &engine {
        if !matches!(
            eng.as_str(),
            "matrix" | "bola" | "bfla" | "ratelimit" | "fuzz"
        ) {
            eprintln!("error: unknown --engine `{eng}`");
            return ExitCode::from(2);
        }
    }
    ...
    let fail_on_sev = match fail_on.as_deref() {
        ...
        Some(other) => {
            eprintln!("error: bad --fail-on `{other}` (use critical|high|medium|low)");
            return ExitCode::from(2);
        }
    };
    ...
    let records: Records = if let Some(path) = annotations.as_deref() {
        match std::fs::read_to_string(path) {
            ...
            Ok(t) => {
                match serde_json::from_str::<AnnotationsFile>(&t) {
                    Ok(a) if a.fp_version == FP_VERSION => a.records,
                    Ok(_) => {
                        eprintln!("error: annotations `{path}` fpVersion mismatch (expected {FP_VERSION})");
                        return ExitCode::from(2);
                    }
                    Err(e) => {
                        eprintln!("error: corrupt annotations `{path}`: {e}");
                        return ExitCode::from(2);
                    }
                }
            }
            ...
            Err(e) => {
                eprintln!("error: cannot read annotations `{path}`: {e}");
                return ExitCode::from(2);
            }
        }
    } else {
    ...
    if want("matrix") {
        ...
        let mat = f
            .iter()
            .filter(|x| matches!(x.engine, EngineId::Matrix))
            .count();
        let orac = f
            .iter()
            .filter(|x| matches!(x.engine, EngineId::Oracle))
            .count();
    ...
    if want("ratelimit") {
        let (f, e) =
            qa_touchstone_core::security::ratelimit::run_ratelimit(&cfg, env.as_deref()).await;
        engines.push(EngineSummary {
            engine: "ratelimit".into(),
            ran: true,
            findings: f.len(),
            errors: e.len(),
        });
    ...
    let redacted: Vec<Finding> = findings
        ...
        .map(|f| Finding {
            ...
            title: red.redact_str(&f.title),
            path: red.redact_str(&f.path),
            evidence: red.redact_str(&f.evidence),
            method: f.method.clone(),
            endpoint: f.endpoint.as_deref().map(|s| red.redact_str(s)),
            identity: f.identity.as_deref().map(|s| red.redact_str(s)),
        })
        .collect();
    ...
    let rfs: Vec<RFinding> = redacted
        ...
        .map(|f| {
            ...
            RFinding {
                fp: fingerprint(f).0,
                engine: engine_str(f.engine).into(),
                severity: sev_str(f.severity).into(),
                rule_id: f.rule_id.clone(),
                oracle: f.oracle.clone(),
                title: f.title.clone(),
                path: f.path.clone(),
    ...
    let baseline_snapshot: Option<Snapshot> = if let Some(path) = baseline.as_deref() {
        match std::fs::read_to_string(path) {
            ...
            Ok(t) => match serde_json::from_str::<Snapshot>(&t) {
                ...
                Ok(_) => {
                    eprintln!(
                        "error: baseline `{path}` fpVersion mismatch (expected {FP_VERSION})"
                    );
                    return ExitCode::from(2);
                }
                Err(e) => {
                    eprintln!("error: corrupt baseline `{path}`: {e}");
                    return ExitCode::from(2);
                }
            },
    ...
    let baseline_items: Vec<_> = baseline_snapshot
        .as_ref()
        .map(|s| s.items.clone())
        .unwrap_or_default();
    let scope_mismatch = baseline_snapshot
        .as_ref()
        .map(|b| !b.scope_hash.is_empty() && b.scope_hash != scope_hash)
        .unwrap_or(false);
    if scope_mismatch {
    ...
    if use_json {
        ...
        println!(
            "security scan: {} finding(s), {}E — {}C {}H {}M {}L {}I",
            report.findings.len(),
        ...
        for f in &report.findings {
            println!(
                "  {} [{}] {}  {}  — {}",
                f.fp, f.severity, f.engine, f.path, f.evidence
            );
        }
    ...
    let model = build_report(
        &current.items,
        &baseline_items,
        report_engines(&report.engines),
        fail_on_sev,
        scope_mismatch,
        "cli",
        &records,
    ...
    if let Some(p) = sarif.as_deref() {
        if let Err(e) = std::fs::write(p, report_to_sarif(&model)) {
            eprintln!("error: cannot write SARIF `{p}`: {e}");
            return ExitCode::from(1);
        }
    }
    if let Some(p) = html.as_deref() {
        if let Err(e) = std::fs::write(p, report_to_html(&model)) {
            eprintln!("error: cannot write HTML `{p}`: {e}");
            return ExitCode::from(1);
        }
    }
    if let Some(p) = junit.as_deref() {
        if let Err(e) = std::fs::write(p, report_to_junit(&model)) {
            eprintln!("error: cannot write JUnit `{p}`: {e}");
            return ExitCode::from(1);
        }
    }
    ...
    if update_baseline {
        if !errors.is_empty() {
            eprintln!(
                "warn: not updating baseline — {} engine error(s) this run",
                errors.len()
            );
            return ExitCode::from(1);
        }
...
mod tests {
    ...
    fn scope_descriptor_stable_and_sensitive() {
        ...
        assert_eq!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&b, None),
            "identity reorder must not change the descriptor"
        );
        ...
        assert_ne!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&c, None),
            "a method change must flip the descriptor"
        );
    }
    ...
    fn scope_descriptor_drops_header_values_keeps_keys() {
        use qa_touchstone_core::config::load_config;
        let mk = |hk: &str, hv: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"GET","url":"https://h/r","headers":[{{"key":"{hk}","value":"{hv}"}}]}}],"security":{{"matrix":{{"endpoints":["r"]}}}}}}"#
            )
        };
        ...
        assert_eq!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&b, None),
            "a header VALUE change must NOT affect the descriptor (values are never hashed)"
        );
        let c = load_config(&mk("X-Other", "v1"), &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&c, None),
            "a header KEY change MUST affect the descriptor"
        );
    }
    ...
    fn scope_descriptor_tracks_bola_idvalue_presence() {
        ...
        let mk = |bval: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"a","auth":{{"type":"none"}}}},{{"id":"b","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"GET","url":"https://h/r/PH"}}],"security":{{"bola":{{"tests":[{{"id":"t","request":"r","idLocation":{{"kind":"path","index":1}},"idValues":{{"a":"oa","b":"{bval}"}}}}]}}}}}}"#
            )
        };
    ...
    fn scope_url_strips_userinfo_query_fragment() {
        assert_eq!(
            scope_url("https://u:p@h:8080/x?q=1#frag"),
            "https://h:8080/x"
        );
    ...
    fn scope_descriptor_drops_query_body_and_url_query_values() {
        ...
        let q = |k: &str, v: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"POST","url":"https://h/r","query":[{{"key":"{k}","value":"{v}"}}]}}],"security":{{"matrix":{{"endpoints":["r"]}}}}}}"#
            )
        };
        ...
        assert_eq!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&b, None),
            "a query VALUE change must NOT flip the descriptor"
        );
        let c = load_config(&q("offset", "1"), &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&c, None),
            "a query KEY change MUST flip the descriptor"
        );
        // body CONTENT change => no flip; body MODE change => flip.
        let body = |mode: &str, content: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"POST","url":"https://h/r","body":{{"mode":"{mode}","content":"{content}"}}}}],"security":{{"matrix":{{"endpoints":["r"]}}}}}}"#
            )
        };
        let d = load_config(&body("raw", "hello"), &|_| None).unwrap();
        let e = load_config(&body("raw", "SECRET-BODY"), &|_| None).unwrap();
        assert_eq!(
            build_scope_descriptor(&d, None),
            build_scope_descriptor(&e, None),
            "a body CONTENT change must NOT flip the descriptor"
        );
        let f = load_config(&body("json", "hello"), &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&d, None),
            build_scope_descriptor(&f, None),
            "a body MODE change MUST flip the descriptor"
        );
        // url inline ?query VALUE change => no flip (query stripped); url PATH change => flip.
        let u = |url: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"GET","url":"{url}"}}],"security":{{"matrix":{{"endpoints":["r"]}}}}}}"#
            )
        };
        let g = load_config(&u("https://h/r?token=aaa"), &|_| None).unwrap();
        let h = load_config(&u("https://h/r?token=SECRET-bbb"), &|_| None).unwrap();
        assert_eq!(
            build_scope_descriptor(&g, None),
            build_scope_descriptor(&h, None),
            "a url ?query VALUE change must NOT flip the descriptor (query stripped)"
        );
        let i = load_config(&u("https://h/r2?token=aaa"), &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&g, None),
            build_scope_descriptor(&i, None),
            "a url PATH change MUST flip the descriptor"
        );
    }
    ...
    fn scope_descriptor_omits_literal_secrets() {
        ...
        for sentinel in [
            ...
            assert!(
                !d.contains(sentinel),
                "descriptor must not contain literal secret `{sentinel}`: {d}"
            );
        }
    }
    ...
    fn scope_descriptor_tracks_oracles_config() {
        ...
        let mk = |s: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"GET","url":"https://h/r"}}],"security":{{"matrix":{{"endpoints":["r"]}},"oracles":{{"sensitive":{s}}}}}}}"#
            )
        };
        ...
        assert_ne!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&b, None),
            "toggling oracles.sensitive must flip the descriptor"
        );
    }
    ...
    fn scope_descriptor_bfla_deny_set_and_endpoints_flip_hash() {
        ...
        let mk = |body: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"GET","url":"https://h/r"}}],"security":{{"bfla":{body}}}}}"#
            )
        };
        ...
        assert_ne!(
            build_scope_descriptor(&a, None),
            build_scope_descriptor(&b, None),
            "denySet change must flip descriptor"
        );
        let two_reqs = r#"{"version":1,"environments":[],"identities":[{"id":"x","auth":{"type":"none"}}],"requests":[{"id":"r","method":"GET","url":"https://h/r"},{"id":"s","method":"POST","url":"https://h/s"}],"security":{"bfla":{}}}"#;
        let d = load_config(two_reqs, &|_| None).unwrap();
        let one_req = r#"{"version":1,"environments":[],"identities":[{"id":"x","auth":{"type":"none"}}],"requests":[{"id":"r","method":"GET","url":"https://h/r"}],"security":{"bfla":{}}}"#;
        let e = load_config(one_req, &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&d, None),
            build_scope_descriptor(&e, None),
            "adding a request with bfla.endpoints:[] must flip descriptor"
        );
    }
    ...
    fn scope_descriptor_bfla_request_shape_changes_flip_hash() {
        ...
        let mk_method = |m: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"{m}","url":"https://h/r"}}],"security":{{"bfla":{{}}}}}}"#
            )
        };
        let get = load_config(&mk_method("GET"), &|_| None).unwrap();
        let post = load_config(&mk_method("POST"), &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&get, None),
            build_scope_descriptor(&post, None),
            "method change on a bfla-only request must flip descriptor"
        );
        // privileged flag change must also flip
        let mk_priv = |p: &str| {
            format!(
                r#"{{"version":1,"environments":[],"identities":[{{"id":"x","auth":{{"type":"none"}}}}],"requests":[{{"id":"r","method":"GET","url":"https://h/r","privileged":{p}}}],"security":{{"bfla":{{}}}}}}"#
            )
        };
        let priv_true = load_config(&mk_priv("true"), &|_| None).unwrap();
        let priv_false = load_config(&mk_priv("false"), &|_| None).unwrap();
        assert_ne!(
            build_scope_descriptor(&priv_true, None),
            build_scope_descriptor(&priv_false, None),
            "privileged flag change must flip descriptor"
        );
    }
