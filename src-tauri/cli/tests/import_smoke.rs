...
fn import_postman_exit_0_and_valid_config() {
    ...
    let out = bin().args(["import", "--input", spec.to_str().unwrap()]).output().unwrap();
    ...
    assert!(reqs.iter().any(|r| r["method"] == "GET"),  "GET request present");
    assert!(reqs.iter().any(|r| r["method"] == "POST"), "POST request present");
...
fn import_openapi_exit_0_base_url_set() {
    ...
    assert!(reqs.iter().any(|r| r["body"]["mode"] == "json"), "OpenAPI body uses mode:json");
    // Round-trip
    let json_str = serde_json::to_string(&cfg).unwrap();
    qa_touchstone_core::config::load_config(&json_str, &|_| None)
        .expect("emitted config must pass load_config");
...
fn import_out_flag_writes_file() {
    ...
    let out = bin().args([
        "import", "--input", spec.to_str().unwrap(),
        "--out", out_path.to_str().unwrap(),
    ]).output().unwrap();
    assert_eq!(out.status.code(), Some(0));
    // stdout must be empty (output went to file)
    assert!(out.stdout.is_empty(), "stdout must be empty when --out is used");
    // File must exist and be valid JSON
    let content = std::fs::read_to_string(&out_path).expect("--out file must exist");
    let _: serde_json::Value = serde_json::from_str(&content).expect("--out file must be valid JSON");
...
fn import_base_url_override() {
    ...
    let out = bin().args([
        "import", "--input", spec.to_str().unwrap(),
        "--base-url", "https://staging.pets.io",
    ]).output().unwrap();
    assert_eq!(out.status.code(), Some(0));
    let stdout = String::from_utf8_lossy(&out.stdout);
    let cfg: serde_json::Value = serde_json::from_str(&stdout).unwrap();
    assert_eq!(cfg["globals"]["variables"]["baseUrl"], "https://staging.pets.io",
...
fn import_relative_url_no_base_url_warns_stderr_exit_0() {
    ...
    let out = bin().args(["import", "--input", spec.to_str().unwrap()]).output().unwrap();
    assert_eq!(out.status.code(), Some(0), "must exit 0 even with baseUrl warning");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("baseUrl") && (stderr.contains("warn") || stderr.contains("empty")),
...
fn import_unrecognized_format_exits_2() {
    ...
    let out = bin().args(["import", "--input", spec.to_str().unwrap()]).output().unwrap();
    assert_eq!(out.status.code(), Some(2), "unrecognized format must exit 2");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("Unrecognized format"), "must echo the TS error string: {stderr}");
    assert!(stderr.contains("Postman"), "must mention Postman in error: {stderr}");
}
...
fn import_non_json_exits_2() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("Not valid JSON"), "must echo the TS JSON error string: {stderr}");
    assert!(stderr.contains("YAML"), "must mention YAML in error: {stderr}");
}
...
fn import_missing_input_exits_2() {
    let out = bin().args(["import", "--input", "/tmp/qa-nonexistent-import-2c1f9a.json"]).output().unwrap();
    assert_eq!(out.status.code(), Some(2), "missing file must exit 2");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(stderr.contains("error:") && stderr.contains("cannot read"), "stderr explains: {stderr}");
}
...
fn import_collections_reference_valid_request_ids() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    let cfg: serde_json::Value = serde_json::from_str(&stdout).unwrap();
    let req_ids: std::collections::HashSet<&str> = cfg["requests"].as_array().unwrap()
        .iter().map(|r| r["id"].as_str().unwrap()).collect();
    ...
    for coll in colls {
        for rid in coll["requests"].as_array().unwrap() {
            assert!(req_ids.contains(rid.as_str().unwrap()),
                "collection request ref '{}' must exist in requests[]", rid);
        }
    }
