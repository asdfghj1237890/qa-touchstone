...
fn write_temp(name: &str, c: &str) -> std::path::PathBuf { let mut p = std::env::temp_dir(); p.push(format!("qa_bl_{}_{}", std::process::id(), name)); std::fs::File::create(&p).unwrap().write_all(c.as_bytes()).unwrap(); p }
fn matrix_cfg(base: &str) -> String { format!(r#"{{ "version":1,"environments":[],
...
async fn baseline_new_gates_carried_passes() {
    ...
    Mock::given(method("GET")).and(path("/s")).respond_with(ResponseTemplate::new(200)).mount(&server).await; // anon-allowed = vuln (High)
    ...
    let out = bin().args(["scan","--config",cfg.to_str().unwrap(),"--baseline",bl.to_str().unwrap()]).output().unwrap();
    assert_eq!(out.status.code(), Some(3), "new finding gates: {}", String::from_utf8_lossy(&out.stderr));
    // Bless: write current as baseline => exit 0
    let out = bin().args(["scan","--config",cfg.to_str().unwrap(),"--baseline",bl.to_str().unwrap(),"--update-baseline"]).output().unwrap();
    assert_eq!(out.status.code(), Some(0), "bless writes + exits 0: {}", String::from_utf8_lossy(&out.stderr));
    // Re-run vs the blessed baseline: CARRIED => exit 0
    let out = bin().args(["scan","--config",cfg.to_str().unwrap(),"--baseline",bl.to_str().unwrap()]).output().unwrap();
    assert_eq!(out.status.code(), Some(0), "carried finding does not gate: {}", String::from_utf8_lossy(&out.stdout));
}
...
async fn sarif_written_with_fingerprint() {
    ...
    let out = bin().args(["scan","--config",cfg.to_str().unwrap(),"--sarif",sarif.to_str().unwrap()]).output().unwrap();
    assert_eq!(out.status.code(), Some(3));
    let v: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(&sarif).unwrap()).unwrap();
...
async fn update_baseline_refuses_on_error() {
    ...
    let cfg = write_temp("er.json", r#"{ "version":1,"environments":[],
      "identities":[{"id":"anon","auth":{"type":"none"}}],
      "requests":[{"id":"down","method":"GET","url":"http://127.0.0.1:1/x"}],
      "security":{"matrix":{"endpoints":["down"]}} }"#);
    let bl = write_temp("ne.json", "");
    let out = bin().args(["scan","--config",cfg.to_str().unwrap(),"--baseline",bl.to_str().unwrap(),"--update-baseline"]).output().unwrap();
    assert_eq!(out.status.code(), Some(1), "engine error => no bless, exit 1");
    assert!(std::fs::read_to_string(&bl).unwrap().is_empty(), "baseline not written on error");
}
...
async fn baseline_secret_is_redacted() {
    ...
    let cfg = write_temp("rl.json", &format!(r#"{{ "version":1,"environments":[],
      "identities":[{{"id":"lp","auth":{{"type":"bearer","token":"SUPERSECRET"}}}}],
      "requests":[{{"id":"del","method":"DELETE","url":"{base}/u"}}],
      "security":{{"matrix":{{"endpoints":["del"],"expect":{{"del":{{"lp":"deny"}}}}}}}} }}"#, base=server.uri()));
