...
fn bola_suggest_path_id_human_output() {
    ...
    let out = bin()
        .args(["bola-suggest", "--config", cfg.to_str().unwrap()])
        .output()
        .unwrap();
    ...
    assert!(
        ...
        "numeric after plural → high confidence"
    );
...
fn bola_suggest_query_id_json_output() {
    ...
    let search = arr
        .iter()
        .find(|r| r["request"] == "searchUser")
        .expect("searchUser in output");
...
fn bola_suggest_stub_contains_identity_ids() {
    ...
    let stub = order["stub"]
        .as_object()
        .expect("stub present for getOrder");
...
fn bola_suggest_no_id_request_prints_note() {
    ...
    assert!(
        health["candidates"].as_array().unwrap().is_empty(),
        "no candidates for /health"
    );
    assert!(
        health["note"].is_string(),
        "note present when no candidates"
    );
...
fn bola_suggest_bad_config_exits_2() {
    let cfg = write_temp(
        "bad.json",
        r#"{"version":99,"environments":[],"identities":[],"requests":[]}"#,
    );
    let out = bin()
        .args(["bola-suggest", "--config", cfg.to_str().unwrap()])
        .output()
...
fn bola_suggest_missing_config_exits_2() {
    let out = bin()
        .args([
            "bola-suggest",
            "--config",
            "/tmp/qa-nonexistent-config-2c1f9a.json",
        ])
        .output()
        .unwrap();
...
fn bola_suggest_redacts_auth_secret() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        !stdout.contains("SUPER_SECRET_TOKEN"),
