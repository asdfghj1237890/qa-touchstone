...
async fn ratelimit_no_throttle_is_vuln() {
    ...
    assert_eq!(findings[0].severity, qa_touchstone_core::security::finding::Severity::High); // sensitive
...
async fn ratelimit_all_net_errors_surfaces_error() {
    ...
    assert!(!errors.is_empty(), "0 completed → an EngineError so scan won't report clean");
}
