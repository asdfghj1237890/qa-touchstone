...
fn fuzz_cfg_query(base: &str, identity: Option<&str>) -> String {
    let id_block = identity
        .map(|i| format!(r#","identity":"{i}""#))
        .unwrap_or_default();
    format!(
        r#"{{
...
async fn fuzz_server_error_on_long_payload() {
    ...
    assert_eq!(
        findings[0].endpoint.as_deref(),
        Some("getItem"),
        "endpoint must be the request id"
    );
    assert!(
        !findings[0].path.contains("://"),
        "path must not contain the resolved URL"
    );
}
...
async fn fuzz_error_leak_sql_in_body() {
    ...
    Mock::given(method("GET"))
        .respond_with(ResponseTemplate::new(200).set_body_string(
            r#"{"message":"You have an error in your SQL syntax near \"1\": syntax error"}"#,
        ))
        .mount(&s)
        .await;
    ...
    let leak: Vec<_> = findings
        .iter()
        .filter(|f| f.rule_id == "fuzz:error-leak")
        .collect();
    assert!(
        !leak.is_empty(),
        "must have at least one error-leak finding"
    );
    assert_eq!(
        leak.len(),
        1,
        "dedup: all payloads on the same seed+signal → one finding"
    );
}
...
async fn fuzz_reflected_xss_is_high() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_string(format!(r#"{{"echo":"{}"}}"#, xss_val)),
        )
        .mount(&s)
        .await;
    Mock::given(method("GET"))
    ...
    let (findings, errors) = run_fuzz(&c, None).await;
    assert!(errors.is_empty(), "{errors:?}");
    let reflected: Vec<_> = findings
        .iter()
        .filter(|f| f.rule_id == "fuzz:reflected")
        .collect();
    assert!(!reflected.is_empty(), "must have reflected finding");
    assert_eq!(
        reflected[0].severity,
        Severity::High,
        "xss reflection must be High"
    );
}
...
async fn fuzz_clean_endpoint_no_findings() {
    ...
    assert!(
        findings.is_empty(),
        "clean endpoint must produce no findings: {findings:?}"
    );
}
...
async fn fuzz_missing_request_is_engine_error() {
    ...
    assert!(
        err.contains("ghost"),
        "config must reject unknown request: {err}"
    );
}
...
async fn fuzz_secret_never_in_output() {
    ...
    for f in &findings {
        ...
        assert!(
            ...
            "sentinel must not appear in finding JSON: {serialized}"
        );
    }
    for e in &errors {
        assert!(
            !e.message.contains(sentinel),
            "sentinel must not appear in EngineError: {}",
            e.message
        );
    }
}
...
async fn fuzz_dedup_multiple_payloads_same_signal() {
    ...
    let se: Vec<_> = findings
        .iter()
        .filter(|f| f.rule_id == "fuzz:server-error")
        .collect();
    assert_eq!(
        se.len(),
        1,
        "all payloads on same seed+signal must be deduped to one finding"
    );
    // Evidence must include "(+k more)" where k = 11 (12 payloads - 1 first = 11 extra).
    assert!(
        se[0].evidence.contains("(+"),
        "evidence must contain (+k more): {}",
        se[0].evidence
    );
}
...
fn fuzz_max_seeds_zero_is_config_error() {
    ...
    let err = load_config(j, &|_| None).unwrap_err();
    assert!(
        err.to_lowercase().contains("maxseedsppertarget") || err.contains("maxSeedsPerTarget"),
...
async fn fuzz_two_explicit_seeds_same_name_diff_location() {
    ...
    assert_eq!(
        se.len(),
        2,
        "two seeds with different locations must produce two findings"
    );
    let identities: std::collections::HashSet<String> =
        se.iter().filter_map(|f| f.identity.clone()).collect();
    assert_eq!(
        identities.len(),
        2,
        "identity (loc_token) must differ between the two findings"
    );
}
...
async fn fuzz_explicit_seeds_survive_cap() {
    ...
    let survivor: Vec<_> = findings
        .iter()
        .filter(|f| f.identity.as_deref() == Some("query:q"))
        .collect();
...
async fn fuzz_bad_explicit_body_location_is_engine_error() {
    ...
    assert!(
        findings.is_empty(),
        "a bad body location must not produce a false-clean finding"
    );
    assert!(
        !errors.is_empty(),
        "a bad body location must produce at least one EngineError"
    );
    // The error message must NOT contain the resolved URL (secret-safe).
    for e in &errors {
        assert!(
            !e.message.contains("://"),
            "EngineError must not contain the resolved URL: {}",
            e.message
        );
    }
}
...
fn scope_descriptor_fuzz_templated_body_flips_on_resolved_shape() {
    ...
    let get_seed_toks = |cfg: &qa_touchstone_core::config::Config| -> Vec<String> {
        ...
        let seeds = derive_fuzz_seeds(
            ...
            &req.query,
            body_val.as_ref(),
            fz.targets[0].seeds.as_deref(),
            fz.max_seeds_per_target,
            "r",
            false,
        );
    ...
    assert!(
        toks_a.iter().any(|t| t == "body:a"),
        "cfg_a must have body:a token; got {toks_a:?}"
    );
    assert!(
        toks_b.iter().any(|t| t == "body:b"),
        "cfg_b must have body:b token; got {toks_b:?}"
    );
}
...
async fn fuzz_e2e_engine_flag_and_baseline_gate() {
    ...
    assert!(
        !findings.is_empty(),
        "must produce at least one finding for baseline gate test"
    );
    // Confirm the finding meets the gate threshold (High >= High → gates).
    assert!(
        findings
            .iter()
            .any(|f| f.severity >= qa_touchstone_core::security::finding::Severity::High),
    ...
    for f in &findings {
        assert!(
            !f.path.contains("://"),
            "finding path must not contain the resolved URL: {}",
            f.path
        );
    }
}
