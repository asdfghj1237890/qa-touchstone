...
fn report_sarif_matches_ts() {
    let f: File = serde_json::from_str(include_str!("fixtures/security_report.json")).unwrap();
    let cur = vec![
        item(
            "aa",
            Severity::High,
    ...
    records.insert(
        "aa".into(),
        qa_touchstone_core::security::lifecycle::LifecycleRecord {
            suppressed: true,
    ...
    let names = |d: &roxmltree::Document| {
        d.descendants()
            ...
            .map(|n| {
                (
                    ...
                    n.descendants().any(|c| c.has_tag_name("skipped")),
                )
            })
            .collect::<Vec<_>>()
    };
    assert_eq!(
        names(&rs),
        names(&ts),
        "JUnit testcases (name + has-failure + has-skipped) match TS at fail_on=high"
    );
    let suites = |d: &roxmltree::Document| {
        d.descendants()
