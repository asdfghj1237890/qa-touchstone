...
use qa_touchstone_core::security::bola::{detect_id_location, BolaIdCandidate, DetectableRequest};
...
fn bolasetup_matches_ts() {
    let cases: Vec<Case> =
        serde_json::from_str(include_str!("fixtures/security_bolasetup.json")).unwrap();
    ...
    for c in &cases {
        ...
        let got_cmp: Vec<CandidateIn> = got
            .iter()
            .map(|g| CandidateIn {
