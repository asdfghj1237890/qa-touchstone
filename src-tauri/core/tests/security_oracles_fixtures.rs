...
struct File {
    sensitive: Vec<SensCase>,
    redact: Vec<RedactCase>,
    schema: Vec<SensCase>,
    #[serde(rename = "runBump")]
    run_bump: Vec<ExpF2>,
}
#[derive(serde::Deserialize)]
struct SensCase {
...
fn resp_for(name: &str) -> (Value, Value) {
    // (body, headers) — mirrors gen-fixtures inputs
    match name {
        "jwt" => (
            json!({"token":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.abcdEF"}),
            json!({}),
        ),
        "aws-key" => (json!({"key":"AKIAIOSFODNN7EXAMPLE"}), json!({})),
        "private-key" => (
            json!({"pem":"-----BEGIN RSA PRIVATE KEY-----xxxx"}),
            json!({}),
        ),
        ...
        "secret_as_key" => (
            json!({"eyJhbGciOiJIUzI1NiJ9.eyJhIjoxfQ.sig123":"eyJhbGciOiJIUzI1NiJ9.eyJhIjoxfQ.sig123"}),
            json!({}),
        ),
        "dedup" => (
            json!({"a":{"email":"x@y.com"},"b":{"email":"x@y.com"}}),
            json!({}),
        ),
        _ => (Value::Null, json!({})),
    }
...
fn oracles_sensitive_matches_ts() {
    ...
    for c in &f.redact {
        assert_eq!(redact(&c.s), c.expected, "redact `{}`", c.s);
    }
    for c in &f.sensitive {
        ...
        let mut got: Vec<(String, String, String, String, String)> = fs
            .iter()
            .map(|x| {
                (
                    x.rule_id.clone(),
                    x.oracle.clone(),
                    sev_str(x.severity).to_string(),
                    x.title.clone(),
                    x.evidence.clone(),
                )
            })
            .collect();
        let mut want: Vec<(String, String, String, String, String)> = c
            .findings
            .iter()
            .map(|e| {
                (
        ...
        assert_eq!(
            got, want,
            "sensitive `{}` (ruleId/oracle/severity/title/evidence set)",
            c.name
        );
...
fn oracles_schema_matches_ts() {
    ...
    for c in &f.schema {
        ...
        let mut got: Vec<(String, String, String, String, String)> =
            check_schema(&body, &contract, &OracleConfig::default())
                .iter()
                .map(|x| {
                    (
                        x.rule_id.clone(),
    ...
    let rb_got: Vec<(String, String, String, String)> = {
        let mut v: Vec<_> = run_oracles(
            200,
            &json!({"id":1,"name":"a","tags":["x"],"leak":"u@v.com"}),
            &json!({}),
            Some(&contract),
    ...
    assert!(
        rb_got
            .iter()
            .any(|t| t.0 == "schema:undeclared" && t.3 == "leak" && t.2 == "high"),
        "undeclared leak bumped to high"
    );
}
