...
fn sev_opt(s: &str) -> Option<Severity> {
    matches!(s, "critical" | "high" | "medium" | "low" | "info").then(|| sev_from(s))
}
...
fn lifecycle_matches_ts() {
    ...
    for c in &f.fnv {
        assert_eq!(fnv1a(&c.s), c.expected, "fnv1a({:?})", c.s);
    }
    for c in &f.alias {
        assert_eq!(
            canonical_rule_id(&c.id),
            c.expected,
            "canonical_rule_id({:?})",
            c.id
        );
