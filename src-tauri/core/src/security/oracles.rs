...
fn value_to_match_str(v: &Value) -> Option<String> {
    // rules match strings + numbers
    match v {
...
fn secret_name_values(body: &Value) -> std::collections::BTreeSet<String> {
    ...
    let key_rules: Vec<&Regex> = rules()
        .iter()
        .filter(|r| r.group == "secrets")
    ...
    if body.is_object() || body.is_array() {
        walk_json(body, "", 0, &mut |_p, k, v| {
            if !k.is_empty() && key_rules.iter().any(|re| re.is_match(k)) {
                if let Some(s) = value_to_match_str(v) {
                    if !s.is_empty() {
                        vals.insert(s);
                    }
                }
...
fn emit(
    ...
    if !seen.insert(k) {
        return;
    } // BTreeSet::insert → false if already present
    ...
    let ev_src = match value {
        Value::Object(_) | Value::Array(_) | Value::Null => "[object]".to_string(),
    ...
    out.push(Finding {
        engine: EngineId::Oracle,
        severity,
        rule_id: id.into(),
        oracle: "sensitive-data".into(),
        title: title.into(),
        path,
        evidence: redact(&ev_src),
        method: None,
        endpoint: None,
        identity: None,
    });
...
pub fn scan_sensitive(body: &Value, headers: &Value, cfg: &OracleConfig) -> Vec<Finding> {
    ...
    match body {
        Value::Object(_) | Value::Array(_) => walk_json(body, "", 0, &mut |p, k, v| {
            visits.push((p.to_string(), k.to_string(), v.clone()))
        }),
...
pub fn infer_contract(body: &Value) -> Contract {
    ...
    if body.is_object() || body.is_array() {
        walk_json(body, "", 0, &mut |path, _k, value| {
            c.insert(normalize_path(path), (js_type(value).into(), true));
        });
    }
    c
}
...
pub fn check_schema(body: &Value, contract: &Contract, cfg: &OracleConfig) -> Vec<Finding> {
    ...
    if body.is_object() || body.is_array() {
        walk_json(body, "", 0, &mut |path, _k, value| {
            present.insert(normalize_path(path), js_type(value).into());
        });
    }
    ...
    for (p, ty) in &present {
        match contract.get(p) {
            ...
            Some((cty, _)) if ty != cty && cty != "null" && ty != "null" => out.push(mk(
...
pub fn run_oracles(
    ...
    let leak_paths: std::collections::BTreeSet<String> =
        sensitive.iter().map(|f| normalize_path(&f.path)).collect();
    let mut out = sensitive;
    let is2xx = (200..=299).contains(&status);
    if is2xx {
        if let Some(c) = baseline {
            for mut f in check_schema(body, c, cfg) {
                if f.title == "Undeclared field" && leak_paths.contains(&f.path) {
    ...
    if !name_secrets.is_empty() {
        ...
        for f in &mut out {
            for s in &ordered {
                if f.path.contains(s.as_str()) {
                    f.path = f.path.replace(s.as_str(), &redact(s));
                }
            }
        }
    }
...
mod tests {
    ...
    use serde_json::json;
    #[test]
    fn redact_thresholds() {
        assert_eq!(redact("abc"), "•••");
        assert_eq!(redact("abcdefghij"), "abc…<redacted>…ij");
    }
    #[test]
    fn secret_in_key_is_masked_in_path() {
        ...
        assert!(
            jwt.path.contains("…<redacted>…") && !jwt.path.contains("sig123"),
            "path must mask the secret-key"
        );
        assert!(jwt.evidence.contains("…<redacted>…"));
    }
    #[test]
    fn walk_depth_cap_no_panic() {
        let mut v = json!("x");
        for _ in 0..200 {
            v = json!({"a": v});
        }
    ...
    #[test]
    fn secret_in_ancestor_key_is_masked_in_path() {
        // A secret used as an object KEY (ancestor of the matched field) must not leak into the path.
        let body =
            json!({"AKIAIOSFODNN7EXAMPLE": {"token": "eyJhbGciOiJIUzI1NiJ9.eyJhIjoxfQ.sig123"}});
        let fs = scan_sensitive(&body, &json!({}), &OracleConfig::default());
        let jwt = fs.iter().find(|f| f.rule_id == "jwt").expect("jwt finding");
        assert!(
            !jwt.path.contains("AKIAIOSFODNN7EXAMPLE"),
            "ancestor secret key must be redacted in path: {}",
            jwt.path
        );
        assert!(
            jwt.path.contains("…<redacted>…"),
            "path should carry the redaction marker: {}",
            jwt.path
        );
    }
    #[test]
    fn schema_path_redacts_secret_key() {
        // Schema findings must redact a secret used as a key too (raw path previously leaked it).
        let fs = check_schema(
            &json!({"AKIAIOSFODNN7EXAMPLE": "v"}),
            &Contract::new(),
            &OracleConfig::default(),
        );
        let f = fs
            .iter()
            .find(|f| f.rule_id == "schema:undeclared")
            .expect("undeclared finding");
        assert!(
            !f.path.contains("AKIAIOSFODNN7EXAMPLE"),
            "schema path must redact a secret key: {}",
            f.path
        );
    }
    #[test]
    fn null_secret_field_evidence_is_object() {
        // TS parity (typeof null === 'object'): a secret-named key with a null value → "[object]" → 8 bullets.
        let fs = scan_sensitive(
            &json!({"password": null}),
            &json!({}),
            &OracleConfig::default(),
        );
        let f = fs
            .iter()
            .find(|f| f.rule_id == "secret-name")
            .expect("secret-name finding");
        assert_eq!(
            f.evidence, "••••••••",
            "null value redacts as 8 bullets like TS String('[object]')"
        );
    }
    #[test]
    fn secret_name_value_reused_as_key_is_masked_in_path() {
        ...
        for f in &fs {
            assert!(
                !f.path.contains("hunter2longvalue"),
                "secret value reused as key leaked in {} path: {}",
                f.rule_id,
                f.path
            );
        }
        assert!(
            fs.iter().any(|f| f.rule_id == "email"),
            "email finding present"
        );
        assert!(
            fs.iter().any(|f| f.rule_id.starts_with("schema:")),
            "schema finding present"
        );
    }
}
