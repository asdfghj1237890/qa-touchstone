...
fn location_of(f: &Finding) -> String {
    format!(
        ...
        f.endpoint.as_deref().unwrap_or(""),
        f.identity.as_deref().unwrap_or("")
    )
}
...
pub fn fingerprint(f: &Finding) -> (String, String) {
    let material = format!(
        "{}|{}|{}|{}",
        engine_token(f.engine),
        canonical_rule_id(&rule_id_of(f)),
        location_of(f),
...
pub fn detail_hash(f: &Finding) -> String {
    fnv1a(&format!("{} {}", f.title, f.evidence))
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Snapshot {
    #[serde(rename = "fpVersion")]
    pub fp_version: u32,
    #[serde(rename = "runId", default)]
    pub run_id: String,
    #[serde(rename = "createdAt", default)]
    pub created_at: String,
    #[serde(rename = "scopeHash", default)]
...
pub struct LifecycleRecord {
    #[serde(default)]
    pub suppressed: bool,
    #[serde(default, rename = "suppressReason")]
    pub suppress_reason: String,
    #[serde(default)]
    pub status: String, // "" => "open" (normalized later in report::ann_of, per TS `r.status||'open'`)
    #[serde(default)]
    pub owner: String,
    #[serde(default)]
    pub note: String,
    #[serde(default, rename = "severityOverride")]
...
pub struct AnnotationsFile {
    #[serde(rename = "fpVersion", default)]
    pub fp_version: u32,
    #[serde(default)]
    pub records: Records,
}
...
pub fn snapshot_of(
    ...
    for f in findings {
        ...
        if let Some(it) = by_fp.get_mut(&fp) {
            it.count += 1;
            continue;
        }
        ...
        by_fp.insert(
            ...
            SnapshotItem {
                ...
                engine: f.engine,
                rule_id: rule_id_of(f),
                path: normalize_path(&f.path),
                location_label: location_of(f),
                title: f.title.clone(),
...
pub fn diff_runs(
    ...
    for it in baseline {
        if !cur.contains(it.fp.as_str()) {
            out.insert(it.fp.clone(), Presence::Resolved);
        }
    }
    out
}
...
pub fn diff_detail(current: &[SnapshotItem], baseline: &[SnapshotItem]) -> BTreeSet<String> {
    let base: BTreeMap<&str, &str> = baseline
        .iter()
        .map(|i| (i.fp.as_str(), i.dfp.as_str()))
        .collect();
    let mut changed = BTreeSet::new();
    for it in current {
        if let Some(d) = base.get(it.fp.as_str()) {
            if *d != it.dfp {
                changed.insert(it.fp.clone());
            }
        }
    }
    changed
...
pub fn effective_severity(f: &Finding, severity_override: Option<Severity>) -> Severity {
    severity_override.unwrap_or(f.severity)
}
...
pub fn scope_hash_of(canonical_descriptor_json: &str) -> String {
    fnv1a(canonical_descriptor_json)
}
...
mod tests {
    ...
    #[test]
    fn location_is_collision_free_across_engines() {
        let a = fingerprint(&f(
            EngineId::Bola,
            "bola.cross-object",
            Severity::High,
    ...
    #[test]
    fn gate_count_threshold_and_presence() {
        let mk = |fp: &str, sev: Severity| SnapshotItem {
            fp: fp.into(),
        ...
        diff.insert("b".to_string(), Presence::New);
        assert_eq!(
            gate_count(&items, &diff, Severity::High, &Records::new()),
            1
        );
        assert_eq!(
            gate_count(&items, &diff, Severity::Medium, &Records::new()),
            2
        );
    ...
    #[test]
    fn override_reclassifies_and_moves_the_gate() {
        let finds = vec![f(
            EngineId::Matrix,
            "matrix.deny-bypass",
            Severity::High,
        ...
        assert_eq!(
            gate_count(&s0.items, &d0, Severity::High, &Records::new()),
            1
        ); // New high gates
    ...
    #[test]
    fn annotations_parse_defaults_and_ignores_gui_fields() {
        ...
        assert!(
            aa.suppressed
                && aa.suppress_reason == "ok"
                && aa.severity_override == Some(Severity::Low)
        );
        let bb = &a.records["bb"];
        assert!(
            !bb.suppressed && bb.status == "acknowledged" && bb.owner == "alice" && bb.note == "n"
        );
        let cc = &a.records["cc"];
        assert!(!cc.suppressed && cc.status.is_empty() && cc.severity_override.is_none());
        // missing => defaults
    }
}
