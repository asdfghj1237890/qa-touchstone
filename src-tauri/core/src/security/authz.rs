...
fn status_field(k: &str) -> bool {
    matches!(
        k,
        "status" | "code" | "result" | "outcome" | "errorcode" | "error_code" | "reason_code"
    )
}
fn prose_field(k: &str) -> bool {
    matches!(
        k,
        "message"
            | "msg"
...
fn error_marker_key(k: &str) -> bool {
    matches!(k, "error" | "errors" | "errorcode" | "error_code")
}
...
pub fn classify_endpoint(method: &str, path: &str) -> bool {
    ...
    path_token_re()
        .split(&path.to_lowercase())
        .filter(|t| !t.is_empty())
        .any(|t| ADMIN_PATH_TOKENS.contains(&t))
}
...
fn value_denies(key: &str, value: &Value) -> bool {
    ...
    if deny_phrase_re().is_match(s) {
        return true;
    }
    if (status_field(key) || key == "error") && deny_token_re().is_match(s) {
        return true;
    }
    false
}
fn scan_for_deny_phrase(node: &Value, depth: u32) -> bool {
    ...
    match node {
        ...
        Value::Object(o) => o.iter().any(|(raw, value)| {
            ...
            ((status_field(&key) || prose_field(&key) || key == "error")
                && value_denies(&key, value))
                || ((value.is_object() || value.is_array())
                    && scan_for_deny_phrase(value, depth + 1))
        }),
        _ => false,
    }
}
...
fn has_error_marker(body: &serde_json::Map<String, Value>) -> bool {
    body.iter()
        .any(|(raw, v)| error_marker_key(&raw.to_lowercase()) && is_truthy_marker(v))
}
...
pub fn detect_soft_deny(body: &Value) -> Option<SoftOutcome> {
    match body {
        Value::Null => None,
        Value::String(s) => (deny_phrase_re().is_match(s) || deny_token_re().is_match(s.trim()))
            .then_some(SoftOutcome::Denied),
        Value::Object(o) => {
            if scan_for_deny_phrase(body, 0) {
                Some(SoftOutcome::Denied)
            } else if has_error_marker(o) {
                Some(SoftOutcome::Error)
            } else {
                None
            }
        }
...
pub fn verdict_for(expectation: Expectation, outcome: Outcome) -> Option<Verdict> {
    match expectation {
        ...
        Expectation::Allow => Some(if outcome == Outcome::Allowed {
            Verdict::Pass
        } else {
            Verdict::Fail
        }),
        Expectation::Deny => Some(if outcome == Outcome::Denied {
            Verdict::Pass
        } else {
            Verdict::Vuln
        }),
...
pub fn default_expectation(
    ...
    if endpoint_priv && !identity_privileged {
        return Expectation::Deny;
    }
    Expectation::Allow
}
...
mod tests {
    ...
    fn outcome_basic() {
        assert_eq!(
            classify_outcome(Some(200), &DEFAULT_DENY_SET),
            Outcome::Allowed
        );
        assert_eq!(
            classify_outcome(Some(403), &DEFAULT_DENY_SET),
            Outcome::Denied
        );
        assert_eq!(
            classify_outcome(Some(500), &DEFAULT_DENY_SET),
            Outcome::Other
        );
        assert_eq!(classify_outcome(None, &DEFAULT_DENY_SET), Outcome::Other);
    }
    #[test]
    fn soft_deny_200_body() {
        assert_eq!(
        ...
        assert_eq!(
            classify_response_outcome(Some(200), &json!({"id":1}), &DEFAULT_DENY_SET),
            Outcome::Allowed
        );
        assert_eq!(
            classify_response_outcome(Some(200), &json!({"error":"boom"}), &DEFAULT_DENY_SET),
            Outcome::Other
        );
    }
    #[test]
    fn endpoint_priv() {
        assert!(classify_endpoint("DELETE", "/u"));
        assert!(classify_endpoint("GET", "/admin/x"));
        assert!(!classify_endpoint("GET", "/u"));
        assert!(endpoint_privileged(Some(true), "GET", "/u"));
    }
    #[test]
    fn endpoint_privileged_override() {
        assert!(
            !endpoint_privileged(Some(false), "DELETE", "/u"),
            "explicit false overrides the write-method heuristic"
        );
        assert!(
            endpoint_privileged(None, "DELETE", "/u"),
            "no override → heuristic (DELETE = privileged)"
        );
