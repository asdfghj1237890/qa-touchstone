...
use crate::security::authz::{classify_response_outcome, default_expectation, endpoint_privileged, verdict_for, Outcome, Verdict, DEFAULT_DENY_SET};
...
pub async fn run_matrix(cfg: &Config, env: Option<&str>) -> Vec<Finding> {
    ...
    let deny_set: Vec<i64> = if sec.deny_set.is_empty() { DEFAULT_DENY_SET.to_vec() } else { sec.deny_set.clone() };
    ...
    for eid in endpoint_ids {
        ...
        for identity in &cfg.identities {
            ...
            if expectation == Expectation::Skip { continue; }
