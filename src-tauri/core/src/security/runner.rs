...
use crate::security::authz::{
    classify_response_outcome, default_expectation, endpoint_privileged, verdict_for, Verdict,
};
...
pub async fn run_security_step(
    cfg: &Config,
    req: &Request,
    identity: &Identity,
    env: Option<&str>,
...
pub async fn run_matrix(cfg: &Config, env: Option<&str>) -> (Vec<Finding>, Vec<EngineError>) {
    let sec = match cfg.security.as_ref().and_then(|s| s.matrix.as_ref()) {
        Some(m) => m,
        None => return (vec![], vec![]),
    };
    ...
    let endpoint_ids: Vec<&String> = if sec.endpoints.is_empty() {
        cfg.requests.iter().map(|r| &r.id).collect()
    } else {
        sec.endpoints.iter().collect()
    };
    ...
    let oracle_cfg = cfg
        .security
        .as_ref()
        .and_then(|s| s.oracles.as_ref())
        .map(|o| o.resolve())
        .unwrap_or_default();
    let oracles_on = oracle_cfg.sensitive || oracle_cfg.schema;
    for eid in endpoint_ids {
        let req = match cfg.requests.iter().find(|r| &r.id == eid) {
            Some(r) => r,
            None => continue,
        };
        ...
        for identity in &cfg.identities {
            ...
            if let Some(Verdict::Vuln) = verdict_for(expectation, outcome) {
                ...
                let severity = if matches!(method.as_str(), "POST" | "PUT" | "PATCH" | "DELETE") {
                    Severity::Critical
                } else {
                    Severity::High
                };
                findings.push(Finding {
                    engine: EngineId::Matrix,
                    severity,
                    rule_id: "matrix.deny-bypass".into(),
                    oracle: "matrix-cell".into(),
                    title: "Access-control bypass (deny expected, access allowed)".into(),
                    path: format!("{} {}", method, eid),
                    evidence: format!(
                        "identity `{}` reached `{} {}` (expected deny; status {})",
                        identity.id, method, eid, step.status
                    ),
                    method: Some(method),
                    endpoint: Some(eid.clone()),
                    identity: Some(identity.id.clone()),
