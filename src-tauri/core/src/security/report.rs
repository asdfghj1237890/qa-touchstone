...
use crate::security::lifecycle::{diff_runs, LifecycleRecord, Presence, Records, SnapshotItem};
...
pub fn build_report(
    ...
    let cur_fps: std::collections::BTreeSet<&str> = current.iter().map(|i| i.fp.as_str()).collect();
    for it in baseline {
    ...
    for f in &findings {
        ...
        if is_gate_failure(f, fail_on) {
            s.gated += 1;
        } // New ∧ sev>=fail_on ∧ !suppressed — matches the exit gate + JUnit
    }
    ReportModel {
        meta: ReportMeta {
            run_id: run_id.into(),
...
pub fn report_to_sarif(model: &ReportModel) -> String {
    ...
    let mut worst: std::collections::BTreeMap<String, Severity> = std::collections::BTreeMap::new();
    for f in &current {
...
pub fn report_to_junit(model: &ReportModel) -> String {
    ...
    let total_fail = current
        .iter()
        .filter(|f| is_gate_failure(f, fail_on))
        .count();
    ...
    for f in &current {
        ...
        if !order.iter().any(|e| engine_token(*e) == tok) {
            order.push(f.engine);
        }
...
pub fn report_to_html(model: &ReportModel) -> String {
    let h = html_escape;
    let s = &model.summary;
    let sev_chip = |n: usize, name: &str| {
        if n > 0 {
            format!("<span class=\"chip sev-{name}\">{n} {name}</span>")
        } else {
            String::new()
        }
    };
    let chips = format!(
        "{}{}{}{}{}",
        sev_chip(s.critical, "critical"),
        sev_chip(s.high, "high"),
        sev_chip(s.medium, "medium"),
        sev_chip(s.low, "low"),
        sev_chip(s.info, "info")
    );
    let eng_rows: String = model
        .engines
        .iter()
        .map(|e| {
            format!(
...
mod tests {
    ...
    fn location_to_uri_transforms() {
        assert_eq!(location_to_uri("GET /foo · bar"), "security/GET//foo/bar");
    ...
    fn pascal_case_splits_on_dots_and_dashes() {
        ...
        assert_eq!(
            pascal_case("bola.cross-object"),
            "BolaCorssObject".replacen("CorssObject", "CrossObject", 1)
        );
        assert_eq!(pascal_case(""), "Rule");
    }

    #[test]
    fn junit_failure_set_follows_fail_on() {
        let cur = vec![mk_item(
            "a",
            Severity::Medium,
            EngineId::Matrix,
    ...
    #[test]
    fn junit_cdata_splits_and_sanitizes_controls() {
        ...
        assert!(
            xml.contains("]]]]><![CDATA[>"),
            "]]> must be split across CDATA sections"
        );
        assert!(
            !xml.contains('\u{0007}'),
            "C0 control must be sanitized to U+FFFD"
        );
    ...
    #[test]
    fn html_contains_findings_and_threshold_gate() {
        let cur = vec![mk_item(
            "a",
            Severity::High,
            EngineId::Bola,
        ...
        assert!(
            html.contains("1 new (\u{2265} high)"),
            "threshold-neutral gate label"
        );
        assert!(html.contains("<!doctype html>"));
    }
    #[test]
    fn html_gate_headline_and_summary_exclude_suppressed() {
        let cur = vec![mk_item(
            "aa",
            Severity::High,
            EngineId::Matrix,
        ...
        assert_eq!(
            m1.summary.gated, 0,
            "suppressed New-high must not be counted as gated"
        );
        assert!(
            report_to_html(&m1).contains("0 new (\u{2265} high)"),
            "suppressed finding must not inflate the gate headline"
        );
    }
    #[test]
    fn html_escapes_hostile_fields() {
        let cur = vec![mk_item(
            "a",
            Severity::High,
            EngineId::Matrix,
        ...
        assert!(
            !html.contains("<script>x</script>")
                ...
                && html.contains("&lt;b&gt;"),
            "rule id AND title escaped"
        );
    }

    #[test]
    fn junit_suppressed_is_skipped_not_failure() {
        let cur = vec![mk_item(
            "aa",
            Severity::High,
            EngineId::Matrix,
        ...
        recs.insert(
            ...
            LifecycleRecord {
                suppressed: true,
                suppress_reason: "ok".into(),
                ..Default::default()
            },
        );
        let xml = report_to_junit(&build_report(
            &cur,
            &[],
            vec![],
            Severity::High,
    ...
    #[test]
    fn junit_failure_cdata_includes_note() {
        let cur = vec![mk_item(
            "aa",
            Severity::High,
            EngineId::Matrix,
        ...
        assert!(
            xml.contains("<failure") && xml.contains("see JIRA-1]]>"),
            "note is appended into the failure CDATA body"
        );
    }
    #[test]
    fn html_triage_shows_owner_status_and_dims_suppressed() {
        let cur = vec![mk_item(
            "aa",
            Severity::High,
            EngineId::Bola,
        ...
        let html = report_to_html(&build_report(
            &cur,
            &[],
            vec![],
            Severity::High,
        ...
        assert!(
            html.contains("acknowledged")
                && html.contains("alice")
                && html.contains("(suppressed: ok)")
        );
        assert!(html.contains("class=\"p-new suppressed\""));
    }

    #[test]
    fn sarif_emits_suppressions_and_owner_status() {
        let cur = vec![mk_item(
            "aa",
            Severity::High,
            EngineId::Bola,
