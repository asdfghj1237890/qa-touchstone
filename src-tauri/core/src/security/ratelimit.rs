...
pub struct ThrottleAnalysis {
    ...
    pub header_hit: bool,
    pub throttled: bool,
    pub first_throttled_index: i64,
    pub allowed_before_throttle: i64,
}
...
fn has_throttle_header(headers: &Value) -> bool {
    headers
        ...
        .map(|o| {
            o.keys()
                .any(|k| THROTTLE_HEADERS.contains(&k.to_lowercase().as_str()))
        })
        .unwrap_or(false)
}
...
pub fn detect_throttle_signal(responses: &[BurstResponse]) -> ThrottleSignal {
    ...
    for r in responses {
        ...
        if has_throttle_header(&r.headers) {
            header_hit = true;
        }
        if saw429 && header_hit {
            break;
        }
    }
    ThrottleSignal {
        throttled: saw429 || header_hit,
        saw429,
        header_hit,
    }
}
...
pub fn analyze_throttle(responses: &[BurstResponse]) -> ThrottleAnalysis {
    let (mut completed, mut ok2xx, mut c429, mut allowed_before_throttle) =
        (0i64, 0i64, 0i64, 0i64);
        ...
        match r.status {
            Some(st) if st > 0 => {
                ...
                if st == 429 {
                    ...
                    if !saw429 {
                        saw429 = true;
                        first_throttled_index = idx;
                    }
                } else if (200..=299).contains(&st) {
                    ok2xx += 1;
                    if !saw429 {
                        allowed_before_throttle += 1;
                    }
                }
            }
            _ => {}
    ...
    ThrottleAnalysis {
        ...
        header_hit,
        throttled: saw429 || header_hit,
        first_throttled_index,
        allowed_before_throttle,
    }
}
...
pub fn classify_rate_limit(throttled: bool, completed: i64) -> RateLimitVerdict {
    if throttled {
        ...
        RateLimitVerdict::Vuln
    } else {
        RateLimitVerdict::Inconclusive
    }
}
...
pub fn rate_limit_severity(
    sensitivity: Option<&str>,
    verdict: RateLimitVerdict,
) -> Option<Severity> {
    if verdict != RateLimitVerdict::Vuln {
        return None;
    }
    Some(if sensitivity == Some("sensitive") {
        Severity::High
    } else {
        Severity::Low
    })
...
fn clamp_int(v: Option<i64>, lo: i64, hi: i64) -> i64 {
    v.unwrap_or(lo).max(lo).min(hi)
}
...
pub async fn run_burst(
    ...
    debug_assert!(
        ...
        "run_burst: a launch index was left unfilled"
    );
...
fn rl_finding_for(
    test: &crate::config::RateLimitTest,
    req: &Request,
    idy: &str,
    ...
    if a.completed == 0 {
        return None;
    } // inconclusive — nothing actually landed
    ...
    match rate_limit_strength(&a) {
        Strength::None => {
            let severity =
                rate_limit_severity(test.sensitivity.as_deref(), RateLimitVerdict::Vuln)?;
            Some(Finding {
                engine: EngineId::RateLimit,
                severity,
                rule_id: "ratelimit.none".into(),
                oracle: "rate-limit".into(),
                ...
                identity: Some(format!("{}: {}", test.id, idy)),
            })
        }
        Strength::Weak => {
            ...
            let evidence = if a.saw429 {
                format!(
                    "{} of {} requests succeeded before the first 429 — throttling engages late",
                    a.allowed_before_throttle, sent
                )
            } else {
            ...
            Some(Finding {
                engine: EngineId::RateLimit,
                severity,
                rule_id: "ratelimit.weak".into(),
                oracle: "rate-limit".into(),
                title: "Weak rate limiting".into(),
                path,
                evidence,
                method: Some(method),
                endpoint: Some(test.request.clone()),
...
pub async fn run_ratelimit(cfg: &Config, env: Option<&str>) -> (Vec<Finding>, Vec<EngineError>) {
    let rcfg = match cfg.security.as_ref().and_then(|s| s.rate_limit.as_ref()) {
        Some(r) => r,
        None => return (vec![], vec![]),
    };
    ...
    for test in &rcfg.tests {
        ...
        let identity = match &test.identity {
            Some(id) => match cfg.identities.iter().find(|i| &i.id == id) {
                Some(i) => i,
                None => continue,
            },
            None => &anon,
        ...
        if test.n.map(|v| v != n).unwrap_or(false) {
            eprintln!("warn: rateLimit test `{}` n clamped to {}", test.id, n);
        }
        if test.concurrency.map(|v| v != c).unwrap_or(false) {
            eprintln!(
                "warn: rateLimit test `{}` concurrency clamped to {}",
                test.id, c
            );
        }
        ...
        let completed = responses
            .iter()
            .filter(|r| r.status.map(|s| s > 0).unwrap_or(false))
            .count();
        if completed == 0 {
            ...
            errors.push(EngineError {
                engine: EngineId::RateLimit,
                endpoint: Some(test.request.clone()),
                identity: Some(idy.clone()),
                message: format!(
                    "rate-limit test `{}`: no requests completed ({} sent, all net errors)",
                    test.id,
                    responses.len()
                ),
            });
        ...
        if let Some(f) = rl_finding_for(test, req, &idy, &responses) {
            findings.push(f);
        }
    }
    (findings, errors)
...
mod tests {
    use super::*;
    use serde_json::json;
    fn r(status: Option<i64>, headers: Value) -> BurstResponse {
        BurstResponse {
            status,
            headers,
            time_ms: 0,
    ...
    fn classify_and_severity() {
        ...
        assert_eq!(
            classify_rate_limit(false, 0),
            RateLimitVerdict::Inconclusive
        );
        assert_eq!(
            rate_limit_severity(Some("sensitive"), RateLimitVerdict::Vuln),
            Some(Severity::High)
        );
        assert_eq!(
            rate_limit_severity(None, RateLimitVerdict::Vuln),
            Some(Severity::Low)
        );
        assert_eq!(
            rate_limit_severity(Some("sensitive"), RateLimitVerdict::Pass),
            None
        );
    }
}
