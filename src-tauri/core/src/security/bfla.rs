...
pub fn bfla_plan(endpoints: &[BflaEndpoint], identities: &[BflaIdentity]) -> Vec<BflaPair> {
    ...
    for (ei, ep) in endpoints.iter().enumerate() {
        ...
        for (ii, id) in identities.iter().enumerate() {
            ...
            pairs.push(BflaPair { endpoint_index: ei, identity_index: ii });
        }
    }
    pairs
}
...
pub fn bfla_severity(method: &str, verdict: Verdict) -> Option<Severity> {
    ...
    Some(if is_mutating_method(method) { Severity::Critical } else { Severity::High })
}
...
pub fn bfla_finding(method: &str, request_id: &str, identity_id: &str, verdict: Verdict) -> Option<Finding> {
    ...
    Some(Finding {
        ...
        evidence: format!("{} (non-privileged) invoked a privileged function", identity_id),
...
pub async fn run_bfla(cfg: &Config, env: Option<&str>) -> (Vec<Finding>, Vec<EngineError>) {
    ...
    let reqs: Vec<&Request> = if bcfg.endpoints.is_empty() {
        ...
        cfg.requests.iter().filter(|r| bcfg.endpoints.contains(&r.id)).collect()
    };
    ...
    let endpoints: Vec<BflaEndpoint> = reqs.iter().map(|req| {
    ...
    let bfla_identities: Vec<BflaIdentity> = identities_ref.iter().map(|i| BflaIdentity {
        id: i.id.clone(),
    ...
    for pair in &plan {
        ...
        if !step.success {
            errors.push(EngineError {
                ...
                message: format!("{} {}: {}", method, req_id, step.error.as_deref().unwrap_or("request failed")),
            });
            continue;
        }
...
mod tests {
    ...
    fn ep(method: &str, path: &str, privileged: Option<bool>) -> BflaEndpoint {
        BflaEndpoint { method: method.into(), path: path.into(), privileged }
    }
    fn id(id: &str, privileged: bool) -> BflaIdentity {
        BflaIdentity { id: id.into(), privileged }
    }

    #[test] fn bfla_plan_crosses_priv_ep_with_nonpriv_id() {
    ...
    #[test] fn bfla_plan_explicit_privileged_override() {
    ...
    #[test] fn classify_bfla_cases() {
        ...
        assert_eq!(classify_bfla(Some(500), &json!({}), deny), Verdict::Inconclusive);
        assert_eq!(classify_bfla(None, &json!({}), deny), Verdict::Inconclusive);
        // soft-deny: 200 with error body => denied => pass
        assert_eq!(classify_bfla(Some(200), &json!({"error":"Access denied"}), deny), Verdict::Pass);
    }

    #[test] fn bfla_severity_cases() {
        assert_eq!(bfla_severity("DELETE", Verdict::Vuln), Some(Severity::Critical));
        assert_eq!(bfla_severity("POST", Verdict::Vuln), Some(Severity::Critical));
        assert_eq!(bfla_severity("PUT", Verdict::Vuln), Some(Severity::Critical));
        assert_eq!(bfla_severity("PATCH", Verdict::Vuln), Some(Severity::Critical));
