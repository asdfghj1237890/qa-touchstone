...
use crate::config::{ApiKeyIn, Auth, BodyMode, Identity, Request};
...
pub fn build_request(
    ...
    let mut query: Vec<String> = req
        ...
        .map(|p| {
            format!(
                "{}={}",
                enc(&qa_substitute(&p.key, map, dynamics)),
                enc(&qa_substitute(&p.value, map, dynamics))
            )
        })
        .collect();
    ...
    if let Some(b) = &req.body {
        match b.mode {
            BodyMode::Json => {
                body_raw = Some(qa_substitute(&b.content, map, dynamics));
    ...
    if !(resolved.starts_with("http://") || resolved.starts_with("https://")) {
        return Err(format!(
            "request `{}` resolved to a non-absolute URL `{}` (SP0b requires absolute URLs)",
            req.id, resolved
        ));
    }
    let url = if query.is_empty() {
        ...
        format!(
            "{}{}{}",
            resolved,
            if resolved.contains('?') { '&' } else { '?' },
            query.join("&")
        )
    };

    let mut request = json!({ "method": req.method, "url": url, "header": headers });
    if let Some(b) = body_raw {
        request["body"] = json!({ "mode": "raw", "raw": b });
    }
...
fn apply_auth(
    ...
    match &id.auth {
        ...
        Auth::Basic { username, password } => {
            headers.push(
                json!({ "key": "Authorization", "value": basic_auth_value(username, password) }),
            );
        }
        Auth::ApiKey {
            key,
...
mod tests {
    ...
    fn substitutes_and_requires_absolute_url() {
        ...
        assert_eq!(
            out["request"]["url"],
            serde_json::json!("https://x.example/v1/u")
        );
