...
fn resolve_credentials(
    ...
    if typ == "manual" {
        ...
        let keychain_sk = if inline_sk.is_empty() {
            crate::secrets::get_secret(api_config_id).ok().flatten()
        } else {
            None
        };
        let sk = crate::secrets::resolve_secret(inline_sk, keychain_sk);
        return match (ak.is_empty(), sk) {
            (false, Some(sk)) => Some(Credentials {
    ...
    Some(Credentials {
        ...
        session_token: if parsed.session_token.is_empty() {
            None
        } else {
...
pub async fn execute_postman_request(
    ...
    sensitive_header_names: Option<Vec<String>>,
) -> Value {
    let credentials = api_config_id.as_deref().and_then(|id| {
        resolve_credentials(&load_config_raw(&app), id, selected_profile.as_deref())
    });
    qa_touchstone_core::executor::execute_request(
        ...
        qa_touchstone_core::executor::ExecOptions {
            ...
            sensitive_header_names: sensitive_header_names.unwrap_or_default(),
        },
    )
    .await
}
