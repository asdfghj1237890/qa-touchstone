...
或身分和 baseline 不同，報告會標示「範圍變了」，而不是假裝消失的問題被修好了。
...
單一事實來源，桌面 IPC 與 headless CLI 都用它）與 TypeScript（瀏覽器
fallback）。兩個機制防止它們走鐘：其一，Rust 端只要看到 config JSON 裡有
不認得的欄位就整份拒收；其二，CI 上的 _parity gate_——用凍結的時鐘和固定的
亂數執行 TypeScript 引擎，產出提交進 repo 的「標準答案卷」
（`scripts/gen-fixtures.mjs` → `src-tauri/core/tests/fixtures/`），Rust 測試
必須重現一模一樣的答案，答案過期 CI 直接失敗。secrets 只能以環境變數引用寫進
...
所有訂單」從檢查清單裡拿掉。再檢查一次，訂單問題當然「消失」了——但
app 不會把它標成「已解決」：因為這次的範圍代碼跟基準對不上，報告會直接
標明「**檢查範圍變了**」，提醒大家問題消失是因為沒檢查，不是修好了。
