...
<details>
<summary>findings、指紋、baseline、雙引擎與 AI 怎麼兜在一起——不用術語也能懂，附三天示範故事</summary>

**所有問題回報單長得一模一樣**。六個安全引擎（矩陣、BOLA、BFLA、rate-limit、
conformance、fuzz）各自找不同種類的問題，但找到的每個問題都會改寫成同一張
標準回報單——`UnionFinding`（`src/qa/types.ts`），欄位固定：哪個引擎、多嚴重、
哪條規則、在哪裡、證據是什麼。因為所有引擎講同一種格式，畫面、報告、跨執行
比對只要寫一次就能服務全部六個引擎——之後新增引擎也完全不用動它們。

**每個問題都有指紋**。app 會把問題的「身分」（規則＋位置）雜湊成指紋
（`fp`），並刻意忽略每次執行會變動的部分（例如文字描述與證據）；那些細節另外
用第二個雜湊（`dfp`）記錄。這樣就能分辨三種情況：全新的問題、同一個問題再次
出現、同一個問題但細節變了。規則改名時走別名表（`src/qa/findings.ts`），
所以改名後的規則永遠保有原本的指紋。

**兩個存檔位，誠實比對**。app 只保留兩份快照：釘選的 `baseline`（你認可的
狀態）與 `lastRun`（最新一次）。比對指紋就得出「新增／延續／已消失」標記。
每份快照還存了一個「這次測了什麼」的雜湊（`scopeHash`）；如果這次涵蓋的端點
或身分和 baseline 不同，報告會在「已消失」標記旁邊同時亮出「範圍變了」的
警告，免得測試範圍縮水被誤當成真的修好了。

**人工判讀永不遺失**。分流決定——抑制誤判、覆寫嚴重度、負責人、備註——存在
自己的表裡，以指紋為 key、不屬於任何單次執行，所以重新掃描不會洗掉它們。
格式升級時，舊的分流資料會被隔離到 `legacy` 欄位而不是刪除；至於重掃一次就能
重生的快照，直接重置即可。

**兩個引擎、一個真相**。矩陣引擎存在兩份——Rust core（`src-tauri/core`，
單一事實來源，桌面 IPC 與 headless CLI 都用它）與 TypeScript（fallback：
瀏覽器裡由它掃描，桌面版上核心還沒接手的少數部分也暫時由它負責）。兩個
機制防止它們走鐘：其一，Rust 端只要看到 config JSON 裡有不認得的欄位就
整份拒收；其二，CI 上的 _parity gate_——用凍結的時鐘和固定的亂數執行
TypeScript 引擎，產出提交進 repo 的「標準答案卷」，Rust 測試必須重現一模
一樣的答案，答案過期 CI 直接失敗。secrets 只能以環境變數引用寫進
config（變數不存在是硬錯誤，不會靜默變成空字串），而 core 經 IPC 回傳的所有
內容——findings *和*錯誤訊息——都會先過同一道遮蔽關卡，因為連「連線被拒」
的錯誤訊息都可能夾帶含 API key 的 URL。

**AI 是乘客，不是駕駛**。上面整條 finding pipeline 是完全確定性的——可選的
AI 功能只坐在它的邊緣。資料要送往 LLM 只有一扇門（`src/qa/llm.ts`），組
prompt 之前先遮蔽；回來的答案採防禦性解析，格式不對就收斂成空結果、絕不
重試。AI 分流與 review 的建議寫進自己的型別，永遠不碰 findings、快照或分流
記錄。唯一刻意留的例外是隨選的 AI 敏感資料掃描：它的結果會以真正的 finding
進入 pipeline，但只在你按下按鈕時發生，而且一律標記 `source: 'llm'`。加上
指紋本來就忽略文字描述與證據——恰好是 AI 能影響的欄位——所以 AI 的非確定
性永遠改變不了一個 finding 跨執行的身分。

**示範故事・第一天——問題被抓到**。你經營一個線上商店網站，後台有一個
「查看所有訂單」的功能（網址是 `GET /admin/orders`）。你準備了兩種測試
身分：**訪客**（沒登入的人）和**管理員**。接著在一張表格裡寫下你的期望：
訪客用這個功能應該被擋下、管理員應該可以通過。這張「身分 × 功能」的期望
表格就叫**矩陣**；**矩陣引擎**則是一個自動測試程式，它替表格裡的每一格
真的發出一次請求，再把實際結果拿來跟你的期望對答案。這次對答案發現：
管理員看得到訂單（符合期望），但訪客竟然也看得到全部訂單（跟期望不符）
——引擎就開出一張**回報單**，上面固定記著：哪個引擎抓到的、違反哪條
規則、發生在哪裡、多嚴重、證據是什麼（證據會先打上馬賽克）。app 還會替
每張回報單算出一個**指紋**——像身分證號碼，從「哪條規則＋發生在哪裡」
計算出來、跟描述文字無關，所以同一個問題以後再被抓到，號碼一定相同。
同一次檢查還發現回應裡露出顧客的 email，所以總共兩張回報單。這整批結果
存成一份**快照**（這次檢查的存檔，並附上一個「這次檢查了哪些項目」的
範圍代碼 `scopeHash`）。你把這份存檔釘選成**基準**（baseline，以後每次
檢查都跟它比），然後做兩個人工決定：訂單問題指派給後端同事；email 那張
其實是測試用的假資料，標成「誤判、以後不用再報」。這兩個決定記在一本
獨立的小本子上，用各張回報單的指紋當索引。

**示範故事・第二天——靠指紋比對**。後端同事修了一半：訪客現在看到的
畫面不一樣了，但其實還是拿得到訂單資料。再檢查一次，app 把新結果拿去跟
基準比對：訂單問題的指紋跟昨天一樣（同一條規則、同一個位置），所以標成
「**延續**」——是老問題，不是新問題；而記錄細節的第二個號碼變了，所以
app 還能告訴你「同一個問題，只是長相變了」。email 那張又被抓到，但小本
子裡它的指紋已經標了「誤判」，所以自動保持靜音，不用重新判斷一次。另外
這次冒出一個真正的新問題：網站沒有**速率限制**（不會擋下短時間內狂發
請求的人）。它的指紋在基準裡找不到，所以標成「**新增**」。如果新問題夠
嚴重，團隊的自動把關系統（CI）就會亮紅燈，擋下這次的程式碼變更。

**示範故事・第三天——防止自欺欺人**。同事為了讓紅燈變綠，乾脆把「查看
所有訂單」從檢查清單裡拿掉。再檢查一次，訂單問題當然「消失」了，甚至還會
被標成「已消失」——它的指紋確實不在這次的結果裡。但 app 不會讓這筆帳就
這樣過關：這次的範圍代碼跟基準對不上，報告會在旁邊直接標明
「**檢查範圍變了**」，提醒大家問題消失是因為沒檢查，不是修好了。
最後你把結果匯出成標準格式的安全報告（SARIF，GitHub 可以直接顯示在
安全頁面上）：報告層把快照和小本子上的人工決定合併起來，證據依你選的
遮蔽等級輸出。這就是整套資料設計從頭到尾的運作——從一次網頁請求，到
團隊把關系統的紅綠燈。
